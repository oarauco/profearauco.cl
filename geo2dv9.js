...
(function (window, document) {
  ...
  function getRawObjectRefs(raw) {
    ...
    if (raw.type === 'segment' || raw.type === 'line') {
      return [raw.p1, raw.p2].filter(Boolean);
    }
  ...
  function validateScene(scene) {
    const supportedTypes = new Set(['point', 'midpoint', 'segment', 'line', 'circle', 'polygon', 'measure']);
...
.geo2d-check {
  ...
  function toolUsesAnchorPreview(tool) {
    return ['point', 'segment', 'line', 'circle', 'polygon', 'midpoint', 'measure-distance'].includes(tool);
  }
...
getObjectRefsText(obj) {
  ...
  if (obj.raw.type === 'segment' || obj.raw.type === 'line') {
    return `${obj.raw.p1 || ''}, ${obj.raw.p2 || ''}`;
  }
...
refreshInspector() {
  ...
  this.propTypeEl.value = this.getTypeLabel(obj.raw.type);
  this.propLabelEl.value = obj.raw.label || '';
...
<\/script>`;
    ...
    updateHoverAndPreview(sx, sy, world) {
      ...
      if (this.mode !== 'viewer' && toolUsesAnchorPreview(this.activeTool) && !nearPoint) {
        ...
        if (anchorObject && anchorObject.constraint) {
          ...
          nextPreview = snapped ? { x: snapped.x, y: snapped.y } : { x: world.x, y: world.y };
        } else {
...
if (this.activeTool === 'move') {
  ...
  return;
    ...
    onPointerMove(e) {
      ...
      if (this._dragInfo) {
        const obj = this.model.getObject(this._dragInfo.pointId);

        if (obj && obj.isPointLike() && obj.isDraggable()) {
