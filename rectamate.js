(function () {
  ...
  function renderRecta(svg, config, scale, ticks, host) {
    const rectHeight = Number(config.altura || DEFAULTS.altura);
    const axisY = Math.max(92, Math.min(124, rectHeight - 55));
    ...
    if (config.mostrarTicks !== false) {
      ...
      ticks.forEach((value) => {
        ...
        if (config.mostrarEtiquetas !== false) {
          addNumericLabel(svg, value, config, config.step, x, axisY + 42, "rm-small-label");
        }
      });
    }
