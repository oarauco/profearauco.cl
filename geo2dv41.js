...
(function (window, document) {
  ...
  const InternalObjectAdapter = Object.freeze({
    ...
    type(value) {
      ...
      return String(raw?.type || raw?.kind || '').trim();
    },
  ...
  function validateScene(scene) {
    ...
    for (let i = 0; i < scene.objects.length; i++) {
      ...
      if (!supportedTypes.has(type)) throw new Error(`El objeto "${id}" tiene un tipo no soportado: ${type || '(vacío)'}.`);
    ...
    for (const raw of scene.objects) {
      ...
      if (type === 'point') {
        if (raw.constraint !== undefined) {
          ...
          const parentTypeOk =
            kind === 'on-line'
              ? isLineLikeRawType(parentType)
              : kind === 'on-segment'
                ? parentType === 'segment' || parentType === 'polyline' || parentType === 'polygon'
              : kind === 'on-circle'
                ? parentType === 'circle' || parentType === 'circle-radius'
              : kind === 'vector-end'
          ...
          if (kind === 'vector-end') {
            raw.draggable = false;
          } else if (kind === 'on-segment' && (parentType === 'polyline' || parentType === 'polygon')) {
      ...
      if (type === 'parallel-line' || type === 'perpendicular-line') {
      ...
      if (type === 'area') {
        if (!Array.isArray(raw.points) || raw.points.length < 3) {
          throw new Error(`El área "${raw.id}" debe tener al menos 3 puntos.`);
        }
      ...
      if (type === 'polygon') {
        if (!Array.isArray(raw.points) || raw.points.length < 3) {
          throw new Error(`El polígono "${raw.id}" debe tener al menos 3 puntos.`);
        }
  ...
  const CONSTRUCTION_TO_INTERNAL_TYPES = Object.freeze({
    ...
    'ellipse:center-radii-rotation': 'ellipse',
    'polyline:through-points': 'polyline',
    'area:through-points': 'area',
  ...
  function getPointOnObjectConstraintKind(targetType) {
    if (targetType === 'segment' || targetType === 'polyline' || targetType === 'polygon') return 'on-segment';
    if (targetType === 'line' || targetType === 'parallel-line' || targetType === 'perpendicular-line') return 'on-line';
    if (targetType === 'ray') return 'on-ray';
    if (targetType === 'circle' || targetType === 'circle-radius') return 'on-circle';
    if (targetType === 'ellipse') return 'on-ellipse';
    return '';
  }
  ...
  const CONSTRUCTION_TO_INTERNAL_IMPORTERS = Object.freeze({
    ...
    'ellipse:center-radii-rotation': importEllipseConstruction,
    'polyline:through-points': ctx => importPointListConstruction(ctx, 'polyline'),
    'area:through-points': ctx => importPointListConstruction(ctx, 'area'),
  ...
  const INTERNAL_TO_CONSTRUCTION_BUILDERS = Object.freeze({
    point: buildPointConstruction,
    midpoint: buildMidpointConstruction,
    segment: buildSegmentConstruction,
    line: buildLineConstruction,
    ray: buildRayConstruction,
    vector: buildVectorConstruction,
    'equipollent-vector': buildEquipollentVectorConstruction,
    'parallel-line': raw => buildDerivedLineConstruction(raw, 'parallel-through-point'),
    'perpendicular-line': raw => buildDerivedLineConstruction(raw, 'perpendicular-through-point'),
    circle: buildCircleConstruction,
    'circle-radius': buildCircleRadiusConstruction,
    ellipse: buildEllipseConstruction,
    polyline: raw => buildPointSequenceConstruction(raw, 'polyline'),
    area: raw => buildPointSequenceConstruction(raw, 'area'),
    polygon: raw => buildPointSequenceConstruction(raw, 'polygon'),
    measure: buildMeasureConstruction,
    text: buildTextConstruction
  });

  function buildConstructionObjectFromInternal(raw) {
    if (!raw || typeof raw !== 'object' || Array.isArray(raw)) return null;

    const builder = INTERNAL_TO_CONSTRUCTION_BUILDERS[InternalObjectAdapter.type(raw)];
  ...
  function normalizeInputSceneToInternal(scene) {
    return sceneUsesConstructionSchema(scene) ? importConstructionSceneToInternal(scene) : scene;
  }

  function prepareScene(scene) {
    const normalized = normalizeInputSceneToInternal(scene);
...
.geo2d-check {
  ...
  class GeoFactory {
    static fromRaw(raw) {
      ...
      const type = InternalObjectAdapter.type(raw);
      if (type === 'point' && raw.constraint) return new ConstrainedPoint(raw);
      if (type === 'point') return new FreePoint(raw);
      if (type === 'midpoint') return new MidpointPoint(raw);
      if (type === 'segment') return new SegmentObject(raw);
      if (type === 'line') return new LineObject(raw);
      if (type === 'ray') return new RayObject(raw);
      if (type === 'vector') return new VectorObject(raw);
      if (type === 'equipollent-vector') return new EquipollentVectorObject(raw);
      if (type === 'parallel-line') return new ParallelLineObject(raw);
      if (type === 'perpendicular-line') return new PerpendicularLineObject(raw);
      if (type === 'circle') return new CircleObject(raw);
      if (type === 'circle-radius') return new CircleRadiusObject(raw);
      if (type === 'ellipse') return new EllipseObject(raw);
      if (type === 'polyline') return new PolylineObject(raw);
      if (type === 'area') return new AreaObject(raw);
      if (type === 'polygon') return new PolygonObject(raw);
      if (type === 'measure') return new MeasureObject(raw);
      if (type === 'text') return new TextObject(raw);
  ...
  class SceneModel {
    ...
    replaceScene(scene) {
      this.scene = readSceneInput(scene);
...
gPoints.appendChild(createSvgEl('circle', {
  ...
  function getObjectTypeLabel(value) {
    ...
    if (type === 'polygon') return 'Polígono';
    if (type === 'measure') return raw.measureType === 'angle' ? 'Ángulo' : 'Medida de distancia';
  ...
  function describeObjectRefs(value) {
    ...
    if (type === 'point') {
      ...
      return `Punto dependiente: ${joinObjectIds(InternalObjectAdapter.refs(raw))}`;
    }

    if (type === 'midpoint') return `Punto medio de ${joinObjectIds([raw.p1, raw.p2])}`;
    if (type === 'segment') return `Segmento entre ${joinObjectIds([raw.p1, raw.p2])}`;
    if (type === 'line') return `Recta por ${joinObjectIds([raw.p1, raw.p2])}`;
    if (type === 'ray') return `Semirrecta con origen ${raw.p1 || '—'} y dirección ${raw.p2 || '—'}`;
    if (type === 'vector') return `Vector de ${raw.p1 || '—'} a ${raw.p2 || '—'}`;
    if (type === 'equipollent-vector') return `Equipolente a ${raw.vectorId || '—'} desde ${raw.point || '—'}`;
    if (type === 'parallel-line') return `Paralela a ${raw.objectId || '—'} por ${raw.point || '—'}`;
    if (type === 'perpendicular-line') return `Perpendicular a ${raw.objectId || '—'} por ${raw.point || '—'}`;
    if (type === 'circle') return `Centro ${raw.center || '—'}, pasa por ${raw.through || '—'}`;
    if (type === 'circle-radius') return `Centro ${raw.center || '—'}, radio ${formatNumberShort(raw.radius) || '—'}`;
    if (type === 'ellipse') return `Centro ${raw.center || '—'}, rx ${formatNumberShort(raw.rx) || '—'}, ry ${formatNumberShort(raw.ry) || '—'}`;
    if (type === 'polyline') return `Vértices: ${joinObjectIds(Array.isArray(raw.points) ? raw.points : [])}`;
    if (type === 'area') return `Área de poligonal: ${joinObjectIds(Array.isArray(raw.points) ? raw.points : [])}`;
    if (type === 'polygon') return `Polígono por ${joinObjectIds(Array.isArray(raw.points) ? raw.points : [])}`;
    if (type === 'measure' && raw.measureType === 'angle') return `Ángulo ${joinObjectIds(Array.isArray(raw.of) ? raw.of : [])}`;
    if (type === 'measure') return `Distancia ${joinObjectIds(Array.isArray(raw.of) ? raw.of : [])}`;
  ...
  class Geo2DConstructionControllerV2 {
    ...
    handleEllipseClick(world, sx, sy) {
      ...
      if (editor._pendingPoints.length === 0) {
        ...
        editor.renderAndSync();
        editor.setStatus('Elipse: define la dirección y longitud del semieje X.');
        return;
      }
  ...
  class Geo2DEditor {
    ...
    bindUI() {
      if (this.mode !== 'viewer') {
        ...
        this.propLabelEl?.addEventListener('input', () => {
          ...
          if (!obj) return;
          if (InternalObjectAdapter.type(obj) === 'text') obj.raw.text = this.propLabelEl.value;
        ...
        this.propExtraSelectEl?.addEventListener('change', () => {
          const obj = this.getSelectedObject();
          if (!obj || InternalObjectAdapter.type(obj) !== 'measure' || obj.raw.measureType !== 'angle') return;
    ...
    createHistoryEntry() {
      return {
        scene: this.model.serializeConstruction(),
        ...
        .map(bucket => `
          <div class="geo2d-object-group">
            ...
            ${bucket.objects.map(obj => {
              const isVisible = InternalObjectAdapter.isVisible(obj);
              return `
    ...
    refreshProperties() {
      ...
      if (this.propTypeEl) this.propTypeEl.value = getObjectTypeLabel(obj);
      this.propLabelEl.value = type === 'text' ? (obj.raw.text || '') : (obj.raw.label || '');
      if (this.propRefsEl) this.propRefsEl.value = describeObjectRefs(obj);
      this.propVisibleEl.checked = InternalObjectAdapter.isVisible(obj);
      ...
      try {
        switch (action) {
          ...
          case 'load':
            ...
            break;
          case 'save':
            downloadTextFile(`${slugify(this.model.meta.title)}.geo2d.json`, jsonPretty(this.model.serializeConstruction()));
            this.setStatus('Guardado.');
            break;
          case 'copyjson':
            copyTextToClipboard(jsonPretty(this.model.serializeConstruction())).then(() => this.setStatus('Copiado.'));
            break;
          case 'apply-json':
