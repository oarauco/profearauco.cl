  ...
  if (info) {
    ...
    if (state._pendingPoints.length > 0 && previewAnchor) {
      ...
      if (first && ['segment', 'line', 'circle', 'midpoint', 'measure-distance'].includes(state.activeTool)) {
        ...
        const previewColor = previewColorMap[state.activeTool] || '#2563eb';
...
<script type="application/json" id="${id}-data">${jsonPretty(this.model.serialize())}<\/script>
<script>
