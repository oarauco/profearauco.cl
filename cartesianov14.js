...
(function (window, document) {
  ...
  const PREMIUM_THEME = {
    ...
    colors: ['#2563eb', '#dc2626', '#059669', '#7c3aed', '#d97706', '#0891b2', '#be123c', '#4f46e5']
  };
  ...
  function readBoolAttr(el, names, fallback) {
  ...
  function readTextAttr(el, names, fallback = '') {
    for (const name of names) {
      const raw = el.getAttribute(name);
  ...
  function isPremiumMode(svg) {
    const theme = String(svg.getAttribute('data-theme') || '').trim().toLowerCase();
    return readBoolAttr(svg, ['data-premium'], false) || theme === 'premium';
  }
  ...
  /* ======================================================================== */
  function drawGrid(svg, xmin, xmax, ymin, ymax, sx, sy, modeX, modeY, theme) {
    ...
    bg.setAttribute('fill', theme && theme.bg ? theme.bg : '#fff');
    svg.appendChild(bg);
    ...
    for (let x = Math.ceil(xmin / xt) * xt; x <= xmax + 1e-12; x += xt) {
      ...
      line.setAttribute('stroke', theme && theme.grid ? theme.grid : '#e6e6e6');
    ...
    function drawAxis(x1, y1, x2, y2) {
      ...
      axis.setAttribute('stroke', theme && theme.axis ? theme.axis : '#000');
      axis.setAttribute('stroke-width', theme ? '1.65' : '1.5');
    ...
    const tickStyle = {
      stroke: theme && theme.tick ? theme.tick : '#999',
      len: 6,
      fs: GRAPH_SETTINGS.axisFontSize,
      fill: theme && theme.text ? theme.text : '#555'
    };
  ...
  /* 9) LEYENDA                                                               */
  /* ======================================================================== */
  function drawLegend(svg, funcs, options) {
    if (!Array.isArray(funcs) || !funcs.length) return;

    const opts = Object.assign({ premium: false, align: 'left', y: 10 }, options || {});
    const size = getSvgLogicalSize(svg);
    const box = {
      x: 10,
      y: opts.y,
      pad: 8,
      dy: opts.premium ? 18 : 20,
      sw: 18,
      sh: 3,
      fs: opts.premium ? Math.max(GRAPH_SETTINGS.legendFontSize, 9) : GRAPH_SETTINGS.legendFontSize
    };
    ...
    funcs.forEach((f, i) => {
      ...
      text.setAttribute('fill', opts.premium ? PREMIUM_THEME.text : '#333');
    ...
    const baseX = box.x;
    const finalX = opts.align === 'right' ? Math.max(10, size.w - w - 12) : baseX;
    const offsetX = finalX - baseX;
    ...
    bg.setAttribute('width', w);
    bg.setAttribute('height', h);
    bg.setAttribute('rx', opts.premium ? 6 : 8);
    bg.setAttribute('ry', opts.premium ? 6 : 8);
    bg.setAttribute('fill', opts.premium ? PREMIUM_THEME.panelBg : 'rgba(255,255,255,0.9)');
    bg.setAttribute('stroke', opts.premium ? PREMIUM_THEME.panelStroke : '#ccc');
    bg.setAttribute('pointer-events', 'none');

    let shadowEl = null;
    if (opts.premium) {
      const shadow = createSvgEl('rect');
      shadow.setAttribute('x', baseX + 1.5);
      shadow.setAttribute('y', box.y + 2);
      shadow.setAttribute('width', w);
      shadow.setAttribute('height', h);
      shadow.setAttribute('rx', 6);
      shadow.setAttribute('ry', 6);
      shadow.setAttribute('fill', PREMIUM_THEME.shadow);
      shadow.setAttribute('pointer-events', 'none');
      g.insertBefore(shadow, g.firstChild);
      shadowEl = shadow;
    }

    g.insertBefore(bg, shadowEl ? shadowEl.nextSibling : g.firstChild);
    svg.appendChild(g);
  }

  function drawPremiumPanel(svg, state) {
    if (!state || !state.premium) return;

    const size = getSvgLogicalSize(svg);
    const title = state.title || 'Plano cartesiano';
    const subtitle = state.subtitle || '';
    const curveCount = (state.curves || []).length;
    const explicitPoints = (state.pointsExplicit || []).length;
    const specialPoints =
      (state.pointsAtX || []).length +
      (state.pointsAtY || []).length +
      (state.pointIntersections || []).length;

    const lines = [];
    if (subtitle) lines.push({ text: subtitle, kind: 'muted' });
    lines.push({
      text: 'Vista x [' + trimNum(state.xmin, 3) + ', ' + trimNum(state.xmax, 3) + ']  y [' +
        trimNum(state.ymin, 3) + ', ' + trimNum(state.ymax, 3) + ']',
      kind: 'muted'
    });
    lines.push({
      text: curveCount + ' trazado' + (curveCount === 1 ? '' : 's') +
        (explicitPoints || specialPoints ? ' - puntos activos' : ''),
      kind: 'muted'
    });
    ...
    const titleFs = 13;
    const fs = 9;
    const maxTextLen = Math.max(title.length, ...lines.map(l => l.text.length));
    const width = Math.min(size.w - 24, Math.max(210, maxTextLen * 5.3 + pad * 2));
    const height = pad * 2 + titleFs + 5 + lines.length * 14;
    ...
    bg.setAttribute('fill', PREMIUM_THEME.panelBg);
    bg.setAttribute('stroke', PREMIUM_THEME.panelStroke);
    g.appendChild(bg);

    const titleEl = createSvgEl('text');
    titleEl.setAttribute('x', x + pad);
    titleEl.setAttribute('y', y + pad + titleFs);
    titleEl.setAttribute('font-size', titleFs);
    titleEl.setAttribute('font-weight', '700');
    titleEl.setAttribute('fill', PREMIUM_THEME.axis);
    titleEl.textContent = title;
    g.appendChild(titleEl);

    lines.forEach((line, i) => {
      const text = createSvgEl('text');
      text.setAttribute('x', x + pad);
      text.setAttribute('y', y + pad + titleFs + 17 + i * 14);
      text.setAttribute('font-size', fs);
      text.setAttribute('fill', line.kind === 'muted' ? PREMIUM_THEME.muted : PREMIUM_THEME.text);
  ...
  function buildPointKey(kind, parts) {
    return kind + ':' + (parts || []).map(v => trimNum(Number(v), 6)).join(':');
  }

  function getPointLabelOffset(state, pointKey) {
    if (!state || !pointKey || !state.pointLabelOffsets) return null;
    return state.pointLabelOffsets[pointKey] || null;
  }

  function getPointLabelOptions(state, pointKey) {
    const base = Object.assign({
      dx: GRAPH_SETTINGS.pointLabelOffsetPx,
      dy: -GRAPH_SETTINGS.pointLabelOffsetPx,
      anchor: 'start',
      baseline: 'auto'
    }, state && state.pointLabelOptions ? state.pointLabelOptions : {});

    const saved = getPointLabelOffset(state, pointKey);
    if (saved) {
      base.dx = saved.dx;
      base.dy = saved.dy;
    }
  ...
  function applyLabelPosition(labelEl, map, pointX, pointY, opts) {
    if (!labelEl || !map) return;
    labelEl.setAttribute('x', map.x2px(pointX) + opts.dx);
    labelEl.setAttribute('y', map.y2px(pointY) + opts.dy);
  }
  ...
  function drawDot(svg, map, x, y, color, pointDecimals, labelOptions, pointKey) {
    const opts = Object.assign({}, labelOptions || {});
    ...
    applyLabelPosition(t, map, x, y, opts);
    ...
    if (pointKey) {
      t.setAttribute('data-point-label-key', pointKey);
      t.setAttribute('data-point-x', String(x));
      t.setAttribute('data-point-y', String(y));
      t.style.cursor = 'move';
  ...
  function drawExplicitPoints(svg, map, state) {
    ...
    pts.forEach(p => {
      if (
        ...
        const pointKey = buildPointKey('explicit', [p.x, p.y]);
        drawDot(svg, map, p.x, p.y, '#111', state.pointDecimals, getPointLabelOptions(state, pointKey), pointKey);
      }
    });
  }
    ...
    spec.slice(0, 16).forEach(s => {
      ...
      ys.forEach((y, idx) => {
        if (y >= state.ymin && y <= state.ymax) {
          const pointKey = buildPointKey('pointx', [s.specIndex || 0, s.x0, idx, y]);
          drawDot(svg, map, s.x0, y, '#111', state.pointDecimals, getPointLabelOptions(state, pointKey), pointKey);
        }
      });
    });
  }
    ...
    spec.forEach(s => {
      ...
      xs.forEach((xi, idx) => {
        if (xi >= state.xmin && xi <= state.xmax) {
          const pointKey = buildPointKey('pointy', [s.specIndex || 0, s.y0, idx, xi]);
          drawDot(svg, map, xi, s.y0, '#111', state.pointDecimals, getPointLabelOptions(state, pointKey), pointKey);
        }
      });
    });
  }
  ...
  function drawPointIntersections(svg, map, state) {
    ...
    pairs.forEach(pair => {
      ...
      points.forEach((point, idx) => {
        ...
        drawn.push(point);
        const pointKey = buildPointKey('inter', [pair.curveA.curveIndex, pair.curveB.curveIndex, idx, point.x, point.y]);
        drawDot(svg, map, point.x, point.y, '#111', state.pointDecimals, getPointLabelOptions(state, pointKey), pointKey);
      });
    });
  }
...
if (!appendAdaptiveInterval(xm, ym, x2, y2, out, depth + 1, strictClipCheck)) return false;
  ...
  function redrawOne(svg, state) {
    ...
    if (state.showPanel) {
      drawPremiumPanel(svg, state);
    }

    if (state.showLegend) {
      drawLegend(
        svg,
        state.curves || (state.funcs || []).concat(state.relations || []).concat(state.inequalities || []),
        {
          premium: state.premium,
          align: state.premium ? 'right' : 'left',
          y: state.showPanel && state.premium ? 12 : 10
        }
      );
    }
  }
  ...
  function findDraggablePointLabel(target, svg) {
    let node = target;

    while (node && node !== svg) {
      if (
        typeof node.getAttribute === 'function' &&
        node.getAttribute('data-point-label-key')
      ) {
        return node;
      }
  ...
  function setPointLabelOffset(state, pointKey, dx, dy) {
    if (!state || !pointKey) return;
    if (!state.pointLabelOffsets) state.pointLabelOffsets = Object.create(null);
    state.pointLabelOffsets[pointKey] = { dx, dy };
  }
  ...
  function getLabelDragScale() {
    const rect = rectWH();
    const map = state.map || getSvgLogicalSize(svg);
    const scaleX = rect && rect.width > 0 ? map.w / rect.width : 1;
    const scaleY = rect && rect.height > 0 ? map.h / rect.height : 1;
  ...
  function beginLabelDrag(labelEl, clientX, clientY, dragId) {
    if (!labelEl) return false;

    const pointKey = labelEl.getAttribute('data-point-label-key');
    const pointX = Number(labelEl.getAttribute('data-point-x'));
    const pointY = Number(labelEl.getAttribute('data-point-y'));
    const current = getPointLabelOptions(state, pointKey);
  ...
  function updateLabelDrag(clientX, clientY) {
    if (!labelDrag) return false;

    const scale = getLabelDragScale();
    const dx = labelDrag.startDx + (clientX - labelDrag.startClientX) * scale.scaleX;
    const dy = labelDrag.startDy + (clientY - labelDrag.startClientY) * scale.scaleY;

    setPointLabelOffset(state, labelDrag.key, dx, dy);
    applyLabelPosition(labelDrag.labelEl, state.map, labelDrag.pointX, labelDrag.pointY, { dx, dy });
    return true;
  }
  ...
  function finishLabelDrag() {
    ...
    if (labelDrag.labelEl && labelDrag.labelEl.style) {
      labelDrag.labelEl.style.cursor = 'move';
    }
  ...
  if ('PointerEvent' in window) {
    ...
    add(svg, 'pointerdown', function (e) {
      ...
      const labelEl = findDraggablePointLabel(e.target, svg);
      if (labelEl && beginLabelDrag(labelEl, e.clientX, e.clientY, e.pointerId)) {
        try {
    ...
    add(svg, 'pointermove', function (e) {
      if (labelDrag && e.pointerId === labelDrag.id) {
        updateLabelDrag(e.clientX, e.clientY);
        e.preventDefault();
        return;
      }
  ...
  add(svg, 'mousedown', function (e) {
    ...
    const labelEl = findDraggablePointLabel(e.target, svg);
    if (labelEl && beginLabelDrag(labelEl, e.clientX, e.clientY, 'mouse')) {
      e.preventDefault();
      return;
    }
  ...
  add(window, 'mousemove', function (e) {
    if (labelDrag && labelDrag.id === 'mouse') {
      updateLabelDrag(e.clientX, e.clientY);
      return;
    }
  ...
  add(svg, 'touchstart', function (e) {
    const labelEl = findDraggablePointLabel(e.target, svg);
    if (labelEl && e.touches.length === 1) {
      const touch = e.touches[0];
      if (beginLabelDrag(labelEl, touch.clientX, touch.clientY, 'touch:' + touch.identifier)) {
        e.preventDefault();
        return;
      }
    }
  ...
  add(svg, 'touchmove', function (e) {
    if (labelDrag && String(labelDrag.id).startsWith('touch:')) {
      const identifier = Number(String(labelDrag.id).slice(6));
      const touch = Array.from(e.touches).find(t => t.identifier === identifier);
      if (touch) {
        updateLabelDrag(touch.clientX, touch.clientY);
        e.preventDefault();
      }
      return;
    }
  ...
  add(svg, 'touchend', function (e) {
    if (labelDrag && String(labelDrag.id).startsWith('touch:')) {
      const identifier = Number(String(labelDrag.id).slice(6));
      const stillActive = Array.from(e.touches).some(t => t.identifier === identifier);
      if (!stillActive) {
        finishLabelDrag();
      }
      return;
  ...
  function initOne(svg) {
    ...
    const premium = isPremiumMode(svg);
    const title = readTextAttr(svg, ['data-title', 'data-titulo'], '');
    const subtitle = readTextAttr(svg, ['data-subtitle', 'data-subtitulo'], '');
    const showLegend = readBoolAttr(
      svg,
      ['data-show-legend', 'data-legend', 'data-leyenda'],
      premium
    );
    const showPanel = readBoolAttr(
      svg,
      ['data-show-panel', 'data-panel'],
      premium
    );

    svg.style.touchAction = modeAttr === 'snapshot' ? 'auto' : 'none';
    if (premium) {
      svg.style.fontFamily = "Inter, system-ui, -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif";
    ...
    if (premium) {
      curves.forEach((curve, i) => {
        if (!curve.color || curve.color === '#d32f2f') {
          curve.color = PREMIUM_THEME.colors[i % PREMIUM_THEME.colors.length];
        }
      });
    }
    ...
    const state = {
      ...
      mode: modeAttr,
      premium,
      theme: premium ? PREMIUM_THEME : null,
      title,
      subtitle,
      showLegend,
      showPanel,
      ...
      pointLabelOptions,
      pointLabelOffsets: Object.create(null),
      pointsExplicit: parsePointList(pointsRaw),
      pointsAtX: parsePointXSpec(pxRaw).map((spec, idx) => preparePointSpec(Object.assign({ specIndex: idx }, spec))),
      pointsAtY: parsePointYSpec(pyRaw).map((spec, idx) => preparePointSpec(Object.assign({ specIndex: idx }, spec))),
 ...
 /* ======================================================================== */
  ...
  window.CartesianoV14 = Object.assign(
    {},
    window.CartesianoV14 || {},
    api
  );

  window.Cartesiano = Object.assign(
    {},
    window.Cartesiano || {},
    api
  );

  // Alias legacy: se mantiene para HTML o llamadas antiguas.
