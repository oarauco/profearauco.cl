/* =========================================================
   DIAGRAMAFLUJOV6 — versión unificada
   Integra:
...
(function (window, document) {
  ...
  class FlujoLayout {
    ...
    debeRamificar(nodo, hijos) {
      if (!nodo || !Array.isArray(hijos) || hijos.length <= 1) return false;
      const conexionesLayout = (nodo.salidas || []).filter(c =>
        c.esPrincipal() && hijos.some(h => h.id === c.hacia)
      );
      if (conexionesLayout.length <= 1) return false;
      if (nodo.tipo === "decision") return true;
      if (conexionesLayout.every(c => c.esCaso())) return true;
      if (conexionesLayout.every(c => c.esPrincipal())) return true;
      return false;
    }
    ...
    medir(nodo, visitando = new Set()) {
      ...
      if (this.debeRamificar(nodo, hijos)) {
        const totalWidth = hijos.reduce((acc, h) => acc + h.subtreeWidth, 0) + this.cfg.separacionHorizontal * (hijos.length - 1);
    ...
    ubicar(nodo, centroX, y, visitando = new Set()) {
      if (!nodo) return;
      if (visitando.has(nodo.id)) return;
      visitando.add(nodo.id);
      const extraLateral = this.anchoLateralAsociado(nodo);
      const anchoReservadoNodo = nodo.ancho + extraLateral;
      nodo.x = centroX - anchoReservadoNodo / 2;
      nodo.y = y;
      this.posiciones.set(nodo.id, { x: nodo.x, y: nodo.y, width: nodo.ancho, height: nodo.alto });
      const hijos = this.hijosLayout.get(nodo.id) || [];
      if (!hijos.length) { visitando.delete(nodo.id); return; }
    ...
    ubicarLaterales() {
      ...
      grupos.forEach(items => {
        ...
        const altoTotal = items.reduce((acc, item, idx) => acc + item.lateral.alto + (idx > 0 ? separacion : 0), 0);
        const yCentroGrupoIdeal = posAsociado.y + posAsociado.height / 2 - altoTotal / 2;
        const xDerecha = posAsociado.x + asociado.ancho + this.cfg.lateralCodo + this.cfg.lateralGap;
        const maxAnchoLateral = Math.max.apply(null, items.map(item => item.lateral.ancho));
        const xIzquierda = posAsociado.x - this.cfg.lateralCodo - this.cfg.lateralGap - maxAnchoLateral;
        ...
        items.forEach((item, idx) => {
          ...
          let yLibre = this.buscarYLibreParaLateral(xDerecha, yIdeal, lateral.ancho, lateral.alto, excluirIds);
          const rectDerecha = { x: xDerecha, y: yLibre, width: lateral.ancho, height: lateral.alto };
          const chocaDerecha = this.haySolapeConEstructura(rectDerecha, excluirIds) || this.haySolapeConLaterales(rectDerecha, excluirIds);
          let xFinal = xDerecha;
          let yFinal = yLibre;
          if (chocaDerecha) {
            const yIzq = this.buscarYLibreParaLateral(xIzquierda, yIdeal, lateral.ancho, lateral.alto, excluirIds);
            const rectIzquierda = { x: xIzquierda, y: yIzq, width: lateral.ancho, height: lateral.alto };
            const chocaIzquierda = this.haySolapeConEstructura(rectIzquierda, excluirIds) || this.haySolapeConLaterales(rectIzquierda, excluirIds);
            if (!chocaIzquierda) {
              xFinal = xIzquierda;
              yFinal = yIzq;
            } else {
          ...
          this.posiciones.set(lateral.id, { x: xFinal, y: yFinal, width: lateral.ancho, height: lateral.alto });
    ...
    ajustarNodoFinal() {
      ...
      const centroGlobal = isFinite(minX) && isFinite(maxX) ? (minX + maxX) / 2 : null;
    ...
    calcular() {
      if (!this.diagrama.raiz) return { posiciones: this.posiciones, width: 0, height: 0 };
      this.construirArbolLayout();
      this.diagrama.nodos.forEach(n => { n._medido = false; });
      ...
      return { posiciones: this.posiciones, width: dims.width, height: dims.height };
    }
  }
  ...
  function dibujarConexion(svg, conexion, posDesde, posHacia, cfg, layout, diagrama, rutasOcupadas) {
    ...
    const a2 = nodoDesde?.tipo === "decision" ? escapeDiagonalRombo(a, posHacia, cfg) : a;
    const b2 = nodoHacia?.tipo === "decision" ? escapeDiagonalRombo(b, posDesde, cfg) : b;
    let d = "";
    ...
    if (semantica === "retorno") {
      ...
      d = "M " + a.x + " " + a.y + " L " + a2.x + " " + a2.y + " C " + mx + " " + a2.y + ", " + mx + " " + b2.y + ", " + b2.x + " " + b2.y + " L " + b.x + " " + b.y;
      ...
      const puntos = [clonarPunto(a), { x: x1, y: a.y }, { x: x1, y: b.y }, clonarPunto(b)];
      d = rutaAPath(puntos);
      etiquetaX = x1 + (b.x - x1) / 2;
      etiquetaY = (a.y + b.y) / 2 - cfg.etiquetaOffsetY;
      registrarRutaOcupada(segmentosDeRuta(puntos), rutasOcupadas);
    } else {
      const rutaFinal = elegirRutaSinChoque(a2, b2, layout, conexion, rutasOcupadas, cfg);
      const puntos = [clonarPunto(a), ...rutaFinal.slice(1, -1), clonarPunto(b)];
      const limpia = simplificarRuta(puntos);
      d = rutaAPath(limpia);
      registrarRutaOcupada(segmentosDeRuta(limpia), rutasOcupadas);
      if (limpia.length >= 3) {
        const mid = limpia[Math.floor(limpia.length / 2)];
        etiquetaX = mid.x;
    ...
    svg.appendChild(crearSVG("path", {
      ...
      "stroke-dasharray": (semantica === "anotacion" || semantica === "referencia" || estilo === "punteada") ? "6 4" : "",
      "marker-end": markerPorColor(color)
    }));
    if (etiqueta) dibujarEtiquetaConexion(svg, etiquetaX, etiquetaY, etiqueta, color, cfg);
  }
