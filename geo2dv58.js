...
(function (window, document) {
  ...
  function isObjectAreaVisible(raw) {
    ...
    if (type === 'polygon') return isPolygonFillVisible(raw);
  ...
  function isAreaBearingRawType(type) {
    return type === 'polygon' || isCurveAreaRawType(type);
  }
...
.geo2d-notable-visible {
  ...
  class Geo2DEditor {
    ...
    updateSelectedObjectAreaVisibility(value) {
      ...
      if (selectedPart && type !== 'polygon') return false;
    ...
    ellipse(id, options = {}) {
      ...
      const ellipseObject = applyBuilderPresentation({
        ...
        def: {
          kind: 'center-vertex-covertex',
          center: normalizeBuilderRef(options.center, 'center'),
          vertex: normalizeBuilderRef(options.vertex, 'vertex'),
          coVertex: normalizeBuilderRef(options.coVertex, 'coVertex')
        }
      }, options);
