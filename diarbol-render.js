(function (window, document) {
  ...
  function truncarTextoConElipsis(texto, maxWidth, fontSize, fontWeight) {
    var txt = DC.aTexto(texto, '');
    if (!txt || !maxWidth || aproximarAnchoTexto(txt, fontSize, fontWeight) <= maxWidth) {
      return txt;
    }
    ...
    var out = txt;
    while (out.length > 1 &&
           aproximarAnchoTexto(out + ellipsis, fontSize, fontWeight) > maxWidth) {
      out = out.slice(0, -1);
    }

    return out.length > 1 ? out + ellipsis : ellipsis;
  }

  function envolverTexto(texto, maxWidth, fontSize, fontWeight) {
    var txt = DC.aTexto(texto, '').trim();
    if (!txt) return [''];
    if (!maxWidth || aproximarAnchoTexto(txt, fontSize, fontWeight) <= maxWidth) {
      return [txt];
    }
    ...
    palabras.forEach(function (palabra) {
      var candidata = actual ? actual + ' ' + palabra : palabra;
      if (aproximarAnchoTexto(candidata, fontSize, fontWeight) <= maxWidth) {
        actual = candidata;
        return;
      }
      ...
      if (aproximarAnchoTexto(palabra, fontSize, fontWeight) <= maxWidth) {
        actual = palabra;
      } else {
        actual = truncarTextoConElipsis(palabra, maxWidth, fontSize, fontWeight);
      }
    });

    if (actual) lineas.push(actual);
    return lineas.length ? lineas : [txt];
  }

  function limitarLineasConElipsis(lineas, maxLines, maxWidth, fontSize, fontWeight) {
    if (!maxLines || lineas.length <= maxLines) return lineas;

    var out = lineas.slice(0, maxLines);
    out[maxLines - 1] = truncarTextoConElipsis(out[maxLines - 1], maxWidth, fontSize, fontWeight);
    return out;
  }
...
interaction: {
  ...
  function normalizarConfig(config) {
    ...
    var entrada = config || {};
    var themeCfg = (DT && entrada.theme && typeof DT.resolverTema === 'function')
      ? DT.resolverTema(entrada.theme)
      : {};
    var cfg = DC.mergeProfundo(defaults, DC.mergeProfundo(themeCfg, entrada));
    ...
    cfg.edgeStyle.boxMaxWidth = Math.max(cfg.edgeStyle.boxMinWidth, DC.aNumero(cfg.edgeStyle.boxMaxWidth, defaults.edgeStyle.boxMaxWidth));

    cfg.leafBoxStyle.width = Math.max(120, DC.aNumero(cfg.leafBoxStyle.width, defaults.leafBoxStyle.width));
    cfg.leafBoxStyle.minWidth = Math.max(100, DC.aNumero(cfg.leafBoxStyle.minWidth, defaults.leafBoxStyle.minWidth));
    cfg.leafBoxStyle.maxWidth = Math.max(cfg.leafBoxStyle.minWidth, DC.aNumero(cfg.leafBoxStyle.maxWidth, defaults.leafBoxStyle.maxWidth));
    cfg.leafBoxStyle.maxLines = Math.max(1, Math.round(DC.aNumero(cfg.leafBoxStyle.maxLines, defaults.leafBoxStyle.maxLines)));
    cfg.leafBoxStyle.autoFit = cfg.leafBoxStyle.autoFit !== false;
...
function normalizarNodoEntrada(nodo) {
  ...
  function resolverAnchoLeafBox(lines, lbs) {
    if (!lbs.show || !lines || !lines.length) return 0;
    if (!lbs.autoFit) return lbs.width;

    var maxTexto = 0;
    lines.forEach(function (line, i) {
      var fontSize = i === 0 ? lbs.titleFontSize : lbs.valueFontSize;
      var fontWeight = i === 0 ? lbs.titleFontWeight : lbs.valueFontWeight;
      maxTexto = Math.max(maxTexto, aproximarAnchoTexto(line, fontSize, fontWeight));
    });

    var natural = Math.ceil(maxTexto + lbs.paddingX * 2);
    return Math.max(lbs.minWidth, Math.min(lbs.maxWidth, natural));
  }

  function ajustarLineasLeafBox(lines, lbs, width) {
    if (!lines || !lines.length) return [];

    var maxTextWidth = Math.max(24, width - lbs.paddingX * 2);
    var out = [];

    lines.forEach(function (line, i) {
      var fontSize = i === 0 ? lbs.titleFontSize : lbs.valueFontSize;
      var fontWeight = i === 0 ? lbs.titleFontWeight : lbs.valueFontWeight;
      envolverTexto(line, maxTextWidth, fontSize, fontWeight).forEach(function (ln) {
        out.push(ln);
      });
    });

    return limitarLineasConElipsis(
      out,
      lbs.maxLines,
      maxTextWidth,
      lbs.valueFontSize,
  ...
  function medirCajaRama(edgeLabel, edgeValue, es) {
    ...
    var maxTextWidth = Math.max(24, es.boxMaxWidth - es.boxPaddingX * 2);

    lines.forEach(function (ln, i) {
      ln.text = truncarTextoConElipsis(ln.text, maxTextWidth, ln.fontSize, ln.fontWeight);
    ...
    return {
      width: Math.min(es.boxMaxWidth, Math.max(es.boxMinWidth, maxWidth + es.boxPaddingX * 2)),
      height: totalHeight,
      lines: lines
    };
  }
  ...
  function medirArbol(state, cfg) {
    ...
    state.leaves.forEach(function (leaf) {
      ...
      var rawLines = obtenerLineasLeafBox(leaf.leafBox);
      var boxWidth = resolverAnchoLeafBox(rawLines, lbs);
      var lines = ajustarLineasLeafBox(rawLines, lbs, boxWidth);

      leaf._leafBoxLines = lines;
      leaf._leafBoxHeight = calcularAlturaLeafBox(lines, lbs);
      leaf._leafBoxWidth = (lbs.show && lines.length) ? boxWidth : 0;
      ...
      state.maxLeafBoxHeight = Math.max(state.maxLeafBoxHeight, leaf._leafBoxHeight);
    });
  }
  ...
  function calcularLayout(root, state, cfg) {
    ...
    var leafGapReal = Math.max(
      cfg.layout.leafGap,
      state.maxLeafBoxHeight ? state.maxLeafBoxHeight + 18 : cfg.layout.leafGap
    );

    var minTreeHeight = contentTop + contentBottomReserve;
    if (state.leaves.length > 1) {
      minTreeHeight += leafGapReal * (state.leaves.length - 1);
    }
    ...
    var reserveLeafBox = state.maxLeafBoxWidth > 0
      ? state.maxLeafBoxWidth + cfg.layout.leafBoxGap + state.maxNodeRadius
      : 0;
    ...
    state.leafBoxX = state.maxLeafBoxWidth > 0
      ? cfg.layout.paddingLeft + state.maxDepth * state.levelGap + state.maxNodeRadius + cfg.layout.leafBoxGap
      : null;
  ...
  function dibujarLeafBox(group, hitGroup, node, cfg, state) {
    ...
    var boxWidth = node._leafBoxWidth || lbs.width;
