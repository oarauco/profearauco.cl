...
(function (window, document) {
  ...
  function getRegularPolygonRawObjectRefs(raw) {
    return cleanObjectRefs([raw.center, raw.vertex]);
  }
  ...
  function isObjectAreaVisible(raw) {
    ...
    if (isCurveAreaRawType(type)) return isCurveAreaVisible(raw, false);
    return false;
  }
  ...
  function isAreaBearingRawType(type) {
    return isPolygonHostRawType(type) || isCurveAreaRawType(type);
  }
  ...
  function validateRegularPolygonObject(raw, { requirePointRef }) {
    requirePointRef(raw.id, 'center', raw.center);
    requirePointRef(raw.id, 'vertex', raw.vertex);
    ...
    if (raw.points[0] !== String(raw.vertex || '').trim()) {
  ...
  function validateRegularPolygonVertexConstraint(raw, { requireRef }) {
    ...
    if (!(raw.constraint.index >= 1) || !(raw.constraint.index < sides)) {
  ...
  function copyInternalPresentationToConstruction(raw, out) {
    ...
    if (raw.draggable !== undefined) out.draggable = !!raw.draggable;
    if (isAreaBearingRawType(InternalObjectAdapter.type(raw)) && raw.parts && typeof raw.parts === 'object' && !Array.isArray(raw.parts)) {
...
.geo2d-notable-visible {
  ...
  function getComputedPropertiesForObject(raw, resolved) {
    ...
    if (type === 'ellipse' && resolved.kind === 'ellipse') {
    ...
    if (type === 'regular-polygon' && resolved.kind === 'polygon') {
    ...
    if (type === 'polygon' && resolved.kind === 'polygon') {
      return {
    ...
    if (type === 'polyline' && resolved.kind === 'polyline') {
      return {
  ...
  function renderPendingToolPreview(ctx) {
    ...
    if (first && ['segment', 'line', 'ray', 'vector', 'circle', 'midpoint', 'measure-distance', 'regular-polygon'].includes(state.activeTool)) {
  ...
  class Geo2DEditor {
    ...
    getSelectedObjectPropertyState() {
      ...
      const isCircle = type === 'circle' || type === 'circle-radius';
      const isEllipse = type === 'ellipse';
      const hasArea = isPolygon || isCircle || isEllipse;
      const selectedPart = isPolygon ? this.normalizeSelectedPart(this.selectedPart) : null;
      const resolved = hasArea ? this.model.getResolvedObject(obj.id) : null;
  ...
  class Geo2DBuilder {
    ...
    circleCR(id, options = {}) {
      ...
      this.pushObject(applyBuilderPresentation({
        ...
        def: {
          kind: 'center-radius',
          center: normalizeBuilderRef(options.center, 'center'),
          radius: safeNumber(options.radius, NaN)
        }
      }, options));
      return this;
    }
    ...
    regularPolygon(id, options = {}) {
      ...
      for (let index = 1; index < pointIds.length; index++) {
    ...
    openEditor(options = {}) {
      return new Geo2DEditor(
        options.target || options.container || '#geo2d-editor',
        {
          ...options,
          mode: 'editor',
          ...(options.scene ? { scene: normalizePublicScene(options.scene) } : {})
        }
      );
    },
    ...
    openViewer(options = {}) {
      return new Geo2DEditor(
        options.target || options.container || '#geo2d-viewer',
        {
          ...options,
          mode: 'viewer',
          viewerMode: normalizeViewerMode(options.viewerMode || options.mode),
          ...(options.scene ? { scene: normalizePublicScene(options.scene) } : {})
        }
      );
    },
