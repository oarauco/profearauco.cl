...
.geo2d-check {
  ...
  function segmentChainIntersections(chainResolved, otherResolved) {
    const segments = resolvedToBoundarySegments(chainResolved);
    ...
    for (const segment of segments) {
      if (isLinearResolvedKind(otherResolved.kind)) {
        ...
        const otherSegments = resolvedToBoundarySegments(otherResolved);
  ...
  function resolveObjectIntersections(resolvedA, resolvedB) {
    ...
    if (isLinearResolvedKind(resolvedA.kind) && isLinearResolvedKind(resolvedB.kind)) {
      ...
      hits = segmentChainIntersections(resolvedB, resolvedA);
  ...
  this.bindSvgInteractions();
    ...
    findNearestIntersectableObjectAtScreen(sx, sy, excludeId = null) {
      ...
      for (const obj of this.model.objects) {
        ...
        if (resolved.kind === 'line') d2 = this.p2l(sx, sy, resolved.p1, resolved.p2, vp);
        if (resolved.kind === 'ray') d2 = this.p2r(sx, sy, resolved.p1, resolved.p2, vp);
        if (resolved.kind === 'polyline') d2 = this.p2polylineEdgesSquared(sx, sy, resolved.points, vp, false);
        if (resolved.kind === 'polygon') {
          d2 = this.pointInWorldPolygonScreen(sx, sy, resolved.points, vp)
            ? 0
            : this.p2polylineEdgesSquared(sx, sy, resolved.points, vp, true);
