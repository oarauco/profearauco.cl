...
(function (window, document) {
  ...
  function isIntersectableRawType(type) {
    return isDirectionalRawType(type) || type === 'circle' || type === 'circle-radius' || type === 'ellipse';
  }
  ...
  function validateScene(scene) {
    ...
    for (const raw of scene.objects) {
      ...
      if (raw.type === 'equipollent-vector') {
        ...
        if (baseVector.type !== 'vector') {
          throw new Error(`El vector equipolente "${raw.id}" debe referenciar un vector libre.`);
        }
        continue;
      }
...
if (resolved.kind === 'text') {
  ...
  gLabels.appendChild(txt);
    ...
    if (state._pendingPoints.length > 0 && previewAnchor) {
      ...
      if (state.activeTool === 'vector-equipollent' && first && state._hoverObjectId) {
        const hovered = model.getResolvedObject(state._hoverObjectId);
        if (hovered && hovered.kind === 'vector') {
          const dx = hovered.p2.x - hovered.p1.x;
          const dy = hovered.p2.y - hovered.p1.y;
          renderVector(gPreview, vp, width, height, first, {
            x: first.x + dx,
            y: first.y + dy
          }, {
            stroke: previewColor,
...
bindUI() {
  ...
  this.bindSvgInteractions();
    ...
    findNearestFreeVectorAtScreen(sx, sy) {
      const vp = this.getScreenViewport();
      let best = null;
      let bestD2 = Infinity;

      for (const obj of this.model.objects) {
        if (!obj.isVisible() || obj.raw.type !== 'vector') continue;
    ...
    getHoveredObjectForTool(sx, sy) {
      ...
      if (this.activeTool === 'vector-equipollent') {
        if (this._pendingPoints.length === 1) return this.findNearestFreeVectorAtScreen(sx, sy);
        return null;
      }
    ...
    handleConstructionClick(world, sx, sy) {
      ...
      if (this.activeTool === 'vector-equipollent') {
        if (this._pendingPoints.length === 0) {
          ...
          this._toolData = null;
          this.renderAndSync();
          this.setStatus('Vector equipolente: selecciona un vector libre de referencia.');
          return;
        }

        const baseVector = this.findNearestFreeVectorAtScreen(sx, sy);
        if (!baseVector) {
          this.setStatus('Vector equipolente: selecciona un vector libre.', true);
          return;
        }
        ...
        this.model.addObject({
          ...
          point: this._pendingPoints[0],
          vectorId: baseVector.id,
          style: { stroke: '#8b5cf6' }
        });

        this.resetConstructionState();
        this.renderAndSync();
        this.setStatus('Vector equipolente creado.');
        return;
      }
