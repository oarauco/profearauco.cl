(function (window, document) {
  ...
  const MENSAJE_MOTOR_FALTANTE =
    "diagramasvenn-auto.js: no se encontró window.DiagramasVenn. Asegúrate de cargar antes diagramasvenn-render.js";
  ...
  function obtenerAPI() {
    return window.DiagramasVenn || null;
  }

  function motorCompletoDisponible(api) {
    return !!(
      api &&
      typeof api.crearDiagramaConjuntos === "function" &&
      typeof api.crearDiagramaConjuntos2 === "function" &&
      typeof api.crearDiagramaConjuntos3 === "function"
    );
  }
  ...
  function inicializarContenedor(contenedor, api) {
    ...
    if (typeof fn !== "function") {
      console.warn(
        `La función ${def.fnName} no está disponible en window.DiagramasVenn.`,
        contenedor
      );
      return false;
    }
