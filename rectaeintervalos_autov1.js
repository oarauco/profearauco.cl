(function (window, document) {
  ...
  function leerBooleano(el, nombre) {
    const raw = el.getAttribute(nombre);
    if (raw === null) return null;
    const token = String(raw).trim().toLowerCase();
    if (token === "" || token === "true" || token === "1" || token === "si" || token === "yes") {
      return true;
    }
  ...
  function enriquecerConfigDesdeDataset(contenedor, config) {
    const out = { ...config };
    const premium = leerBooleano(contenedor, "data-premium");
    const theme = contenedor.getAttribute("data-theme");

    if (premium !== null && out.premium === undefined) out.premium = premium;
    if (theme && out.theme === undefined) out.theme = theme;
  ...
  function estimarAlturaCanvas(config, fallbackHeight) {
    ...
    const extraTop = Math.max(maxBaseHeight, solutionHeight) + labelReserve;
  ...
  function inicializarContenedor(contenedor, api, options = {}) {
    ...
    const config = enriquecerConfigDesdeDataset(
      contenedor,
      parsearJSONDeAtributo(contenedor, "data-config", {})
    );
