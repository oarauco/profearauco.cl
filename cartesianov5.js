...
function drawFunctionFromExpr(svg, expr, map, xmin, xmax, ymin, ymax, color, doClip) {
  ...
  function appendAdaptiveInterval(x1, y1, x2, y2, out, depth = 0) {
    ...
    if (!appendAdaptiveInterval(x1, y1, xm, ym, out, depth + 1)) return false;
    if (!appendAdaptiveInterval(xm, ym, x2, y2, out, depth + 1)) return false;
  ...
  for (let x = xmin; x <= xmax + 1e-12; x += step) {
    ...
    if (!doClip) {
      ...
      const ok = appendAdaptiveInterval(xPrev, yPrev, x, y, seg, 0);
    ...
    if (inPrev && inNow) {
      ...
      if (pInt) {
        ...
        const ok = appendAdaptiveInterval(xPrev, yPrev, pInt[0], pInt[1], seg, 0);
        if (!ok) seg.push(pInt);
      }
      ...
      if (pInt) {
        seg.push(pInt);
        const ok = appendAdaptiveInterval(pInt[0], pInt[1], x, y, seg, 0);
        if (!ok) seg.push([x, y]);
      } else {
        seg.push([x, y]);
      }
      ...
      if (bothAbove || bothBelow) {
        ...
        if (p1 && p2) {
          ...
          const ok = appendAdaptiveInterval(a[0], a[1], b[0], b[1], seg, 0);
          if (!ok) seg.push(b);
  ...
  function attachInteractions(svg, state, onChange) {
    ...
    if ('PointerEvent' in window) {
      ...
      svg.addEventListener('pointerdown', (e) => {
        ...
        if (active.size === 1) {
          ...
          pinchLastDist = Math.hypot(pts.p1.x - pts.p2.x, pts.p1.y - pts.p2.y);
        }
    ...
    svg.addEventListener('touchmove', (e) => {
      if (touchMode === 'pan' && e.touches.length === 1 && lastPanTouch) {
        const t = e.touches[0];
        const dx = t.clientX - lastPanTouch.x;
        const dy = t.clientY - lastPanTouch.y;
  ...
  /* ======================================================================== */
  function init() {
    const svgsToProcess = document.querySelectorAll('svg[id^="grid-"]');
