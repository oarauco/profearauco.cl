...
(function (window, document) {
  ...
  function readEmbeddedSceneSource(sceneSource) {
    if (!sceneSource) return null;

    if (typeof sceneSource === 'string') {
      const el = document.querySelector(sceneSource);
      if (!el) throw new Error('No se encontró sceneSource: ' + sceneSource);
      return (el.getAttribute && (el.getAttribute('data-scene') || el.getAttribute('data-geo2d-scene')) || '').trim() || null;
    }

    if (sceneSource instanceof Element) {
      return (sceneSource.getAttribute && (sceneSource.getAttribute('data-scene') || sceneSource.getAttribute('data-geo2d-scene')) || '').trim() || null;
    }
  ...
  function convertConstructionObjectToLegacy(raw, byId) {
    ...
    if (kind === 'point' && defKind === 'on-object') {
      ...
      if (constraintKind === 'on-segment' && (targetType === 'polyline' || targetType === 'polygon')) {
        const edgeIndex = normalizeEdgeIndex(def.edgeIndex);
        if (edgeIndex !== null) out.constraint.edgeIndex = edgeIndex;
      }
    ...
    if (kind === 'point' && defKind === 'intersection') {
      ...
      const edgeIndex = normalizeEdgeIndex(def.edgeIndex);
      const edgeIndex2 = normalizeEdgeIndex(def.edgeIndex2);
      if (edgeIndex !== null) out.constraint.edgeIndex = edgeIndex;
      if (edgeIndex2 !== null) out.constraint.edgeIndex2 = edgeIndex2;
    ...
    if (kind === 'line' && defKind === 'parallel-through-point') {
      ...
      out.point = String(def.point || '').trim();
      const edgeIndex = normalizeEdgeIndex(def.edgeIndex);
      if (edgeIndex !== null) out.edgeIndex = edgeIndex;
      return out;
    }
  ...
  function buildConstructionObjectFromLegacy(raw) {
    ...
    if (raw.type === 'point' && raw.constraint) {
      ...
      if (constraint.kind === 'intersection') {
        ...
        const edgeIndex = normalizeEdgeIndex(constraint.edgeIndex);
        const edgeIndex2 = normalizeEdgeIndex(constraint.edgeIndex2);
        if (edgeIndex !== null) out.def.edgeIndex = edgeIndex;
        if (edgeIndex2 !== null) out.def.edgeIndex2 = edgeIndex2;
      ...
      const edgeIndex = normalizeEdgeIndex(raw.edgeIndex);
      if (edgeIndex !== null) out.def.edgeIndex = edgeIndex;
  ...
  function loadSceneFromOptions(options = {}) {
    ...
    if (options.target || options.container) {
      const embedded = readEmbeddedSceneSource(options.target || options.container);
...
findObjectsAtScreen(sx, sy) {
  ...
  for (const obj of this.model.objects) {
    ...
    if (resolved.kind === 'area') {
      d2 = this.pointInWorldPolygonScreen(sx, sy, resolved.points, vp)
        ? 0
...
bindUI() {
  ...
  this.bindSvgInteractions();
    ...
    clearHoverAndPreview() {
      ...
      if (this._hoverObjectEdgeIndex !== null) {
        this._hoverObjectEdgeIndex = null;
        dirty = true;
      }
    ...
    updateHoverAndPreview(sx, sy, world) {
      ...
      if (nextHoverObjectEdgeIndex !== this._hoverObjectEdgeIndex) {
        this._hoverObjectEdgeIndex = nextHoverObjectEdgeIndex;
        dirty = true;
      }
    ...
    pointInWorldPolygonScreen(sx, sy, points, vp) {
      if (!points || points.length < 3) return false;
      const screenPts = points.map(p => worldToScreen(vp, this._svgWidth, this._svgHeight, p.x, p.y));
    ...
    getHoveredObjectForTool(sx, sy) {
      ...
      if (this.activeTool === 'vector-equipollent') {
        if (!this._toolData?.vectorObjectId) return this.findNearestVectorAtScreen(sx, sy);
        return this.model.getObject(this._toolData.vectorObjectId);
      }
...
if (this.activeTool === 'move') {
  ...
  return;
    ...
    handleConstructionClick(world, sx, sy) {
      ...
      if (this.activeTool === 'circle-radius') {
        const picked = this.pickOrCreateAnchorPoint(world, sx, sy);
        if (!picked?.id) return;
        ...
        if (!(radius > 1e-9)) {
          this.setStatus('El radio debe ser un número mayor que 0.', true);
          return;
        }
      ...
      if (this.activeTool === 'parallel-line' || this.activeTool === 'perpendicular-line') {
        if (!this._toolData?.referenceObjectId) {
          ...
          if (!referenceObject) {
            this.setStatus('Selecciona un segmento, recta, semirrecta o una arista de poligonal/polígono.', true);
            return;
          }
      ...
      if (this._pendingPoints.length === 1) {
        ...
        if (this.activeTool === 'ray') {
          ...
          this.setStatus('Vector libre: selecciona el punto final.');
        } else {
