(() => {
  ...
  const TEMA_PREMIUM = {
    ...
    textoSuave: "#64748b",
    flecha: "#334155",
    azul: { fondo: "#eff6ff", borde: "#2563eb" },
    verde: { fondo: "#ecfdf5", borde: "#059669" },
    ambar: { fondo: "#fffbeb", borde: "#d97706" },
    violeta: { fondo: "#f5f3ff", borde: "#7c3aed" },
    rojo: { fondo: "#fef2f2", borde: "#dc2626" }
  };
  ...
  function obtenerCanvasYContexto(canvasId) {
    ...
    const logicalWidth = canvas.__oaLogicalWidth || canvas.width;
    const logicalHeight = canvas.__oaLogicalHeight || canvas.height;
    const dpr = canvas.__oaDpr || 1;
    if (ctx && typeof ctx.setTransform === "function") {
      ctx.setTransform(dpr, 0, 0, dpr, 0, 0);
    }
    return {
      canvas: { width: logicalWidth, height: logicalHeight, el: canvas },
      ctx
    };
  }
  function esPremium(config = {}) {
    const tema = String(config.tema || config.theme || "").toLowerCase();
    return config.premium === true || tema === "premium";
  }
  function obtenerTema(config = {}) {
    return esPremium(config) ? TEMA_PREMIUM : null;
  }
  function limpiarCanvas(ctx, canvas, tema) {
  ...
  function dibujarTitulo(ctx, canvas, config = {}, tema) {
    const titulo = String(config.titulo || config.title || "").trim();
    const subtitulo = String(config.subtitulo || config.subtitle || "").trim();
    ...
    if (titulo) {
      ctx.fillStyle = config.colorTitulo || (tema ? tema.texto : "#212529");
      ctx.font = config.fontTitulo || "bold 18px Arial";
      ctx.fillText(titulo, canvas.width / 2, config.yTitulo || 12);
    }
    if (subtitulo) {
      ctx.fillStyle = config.colorSubtitulo || (tema ? tema.textoSuave : "#6c757d");
      ctx.font = config.fontSubtitulo || "12px Arial";
      ctx.fillText(subtitulo, canvas.width / 2, titulo ? (config.yTitulo || 12) + 24 : (config.yTitulo || 12));
    }
    ctx.restore();
  }
  function estiloConjunto(config, sufijo, fallback) {
    const tema = obtenerTema(config);
    return {
      fillStyle: config[`colorFondo${sufijo}`] || fallback.fondo,
      strokeStyle: config[`colorBorde${sufijo}`] || fallback.borde,
      textColor: config[`colorTexto${sufijo}`] || (tema ? tema.texto : "#000"),
      lineWidth: config.grosorConjunto || (tema ? 1.6 : 2),
      font: config.fontConjunto || "bold 16px Arial"
    };
  }
  function estiloNodo(config, sufijo, fallback) {
    const tema = obtenerTema(config);
    return {
      fondo: config[`colorNodo${sufijo}`] || fallback.fondo,
      borde: config[`colorBordeNodo${sufijo}`] || fallback.borde,
      opciones: {
        textColor: config.colorTextoNodo || (tema ? tema.texto : "#000"),
        lineWidth: config.grosorBordeNodo || (tema ? 1.7 : 2),
        font: config.fontNodo || "14px Arial"
      }
    };
  }
  ...
  window.crearDiagramaSagital =
    function (canvasId, config = {}) {
      ...
      limpiarCanvas(ctx, canvas, tema);
      dibujarTitulo(ctx, canvas, config, tema);
      dibujarConjunto(
        ctx,
        centroAx,
        yTop,
        ...
        altoConjunto,
        config.nombreA || "A",
        estiloConjunto(config, "A", tema ? tema.azul : { fondo: "#e7f1ff", borde: "#0d6efd" }),
      );
      dibujarConjunto(
        ctx,
        centroBx,
        ...
        altoConjunto,
        config.nombreB || "B",
        estiloConjunto(config, "B", tema ? tema.verde : { fondo: "#d1e7dd", borde: "#198754" }),
      );
      ...
      (config.pares || []).forEach((par) => {
        ...
        if (origen && destino) {
          dibujarFlechaRecta(ctx, origen, destino, radioNodo, {
            color: config.colorFlecha || (tema ? tema.flecha : "#495057"),
            lineWidth: config.grosorFlecha || 2,
          });
        }
      });
      const nodoA = estiloNodo(config, "A", tema ? tema.azul : { fondo: "#e7f1ff", borde: "#0d6efd" });
      const nodoB = estiloNodo(config, "B", tema ? tema.verde : { fondo: "#d1e7dd", borde: "#198754" });
      Object.values(nodosA).forEach((nodo) =>
        dibujarNodo(ctx, nodo, radioNodo, nodoA.fondo, nodoA.borde, nodoA.opciones),
      );
      Object.values(nodosB).forEach((nodo) =>
        dibujarNodo(ctx, nodo, radioNodo, nodoB.fondo, nodoB.borde, nodoB.opciones),
      );
      if (suspA.inicio)
  ...
  window.crearDiagramaComposicion =
    function (canvasId, config = {}) {
      ...
      dibujarConjunto(
        ...
        altoConjunto,
        config.nombreB || "B",
        estiloConjunto(config, "B", tema ? tema.ambar : { fondo: "#fff3cd", borde: "#fd7e14" }),
      );
      dibujarConjunto(
        ctx,
        centroCx,
        ...
        altoConjunto,
        config.nombreC || "C",
        estiloConjunto(config, "C", tema ? tema.verde : { fondo: "#d1e7dd", borde: "#198754" }),
      );
      dibujarEtiquetaRelacion(
        ctx,
        ...
        (centroAx + centroBx) / 2,
        yTop + 20,
        { font: "italic 15px Arial", color: config.colorRelacionAB || (tema ? tema.textoSuave : "#495057") },
      );
      dibujarEtiquetaRelacion(
        ctx,
        ...
        (centroBx + centroCx) / 2,
        yTop + 20,
        { font: "italic 15px Arial", color: config.colorRelacionBC || (tema ? tema.textoSuave : "#495057") },
      );
      if (config.nombreCompuesta) {
        dibujarEtiquetaRelacion(
          ...
          (centroAx + centroCx) / 2,
          config.yTituloCompuesta || (config.titulo || config.title ? 48 : 24),
          {
      ...
      (config.paresAB || []).forEach((par) => {
        ...
        if (origen && destino) {
          dibujarFlechaRecta(ctx, origen, destino, radioNodo, {
            color: config.colorFlechaAB || (tema ? tema.flecha : "#495057"),
      ...
      (config.paresBC || []).forEach((par) => {
        ...
        if (origen && destino) {
          dibujarFlechaRecta(ctx, origen, destino, radioNodo, {
            color: config.colorFlechaBC || (tema ? tema.flecha : "#495057"),
      ...
      if (config.mostrarCompuesta) {
        ...
        paresCompuestos.forEach((par) => {
          ...
          if (origen && destino) {
            dibujarFlechaRecta(ctx, origen, destino, radioNodo, {
              color: config.colorFlechaCompuesta || (tema ? tema.rojo.borde : "#dc3545"),
      ...
      const nodoA = estiloNodo(config, "A", tema ? tema.azul : { fondo: "#e7f1ff", borde: "#0d6efd" });
      const nodoB = estiloNodo(config, "B", tema ? tema.ambar : { fondo: "#fff3cd", borde: "#fd7e14" });
      const nodoC = estiloNodo(config, "C", tema ? tema.verde : { fondo: "#d1e7dd", borde: "#198754" });
      Object.values(nodosA).forEach((nodo) =>
        dibujarNodo(ctx, nodo, radioNodo, nodoA.fondo, nodoA.borde, nodoA.opciones),
      );
      Object.values(nodosB).forEach((nodo) =>
      ...
      Object.values(nodosC).forEach((nodo) =>
        dibujarNodo(ctx, nodo, radioNodo, nodoC.fondo, nodoC.borde, nodoC.opciones),
      );
      if (suspA.inicio)
        dibujarSuspensivos(ctx, centroAx, yTop + 24, suspA.textoInicio);
      if (suspA.fin)
  ...
  window.crearDiagramaAutoSagital =
    function (canvasId, config = {}) {
      ...
      window.crearDiagramaSagital(canvasId, {
        ...
        colorNodoA: config.colorNodoA,
        colorNodoB: config.colorNodoB,
        colorBordeNodoA: config.colorBordeNodoA,
        colorBordeNodoB: config.colorBordeNodoB,
        colorFondoA: config.colorFondoA,
        colorFondoB: config.colorFondoB,
        colorBordeA: config.colorBordeA,
        colorBordeB: config.colorBordeB,
    ...
    function (canvasId, config = {}) {
      ...
      if (config.nombreRelacion) {
        dibujarEtiquetaRelacion(
          ...
          config.nombreRelacion,
          canvas.width / 2,
          config.yNombreRelacion || (config.titulo || config.title ? 52 : 24),
          { font: "italic 16px Arial", color: tema ? tema.textoSuave : "#212529" },
        );
      }
      pares.forEach(([a, b]) => {
        ...
        if (a === b) {
          dibujarLazo(ctx, origen, radioNodo, {
            color: config.colorFlecha || (tema ? tema.violeta.borde : "#495057"),
            lineWidth: config.grosorFlecha || 2,
          });
          return;
        }
      ...
      Object.values(nodos).forEach((nodo) => {
        dibujarNodo(
          ...
          nodo,
          radioNodo,
          config.colorNodo || (tema ? tema.violeta.fondo : "#e7f1ff"),
          config.colorBordeNodo || (tema ? tema.violeta.borde : "#0d6efd"),
          {
            textColor: config.colorTextoNodo || (tema ? tema.texto : "#000"),
            lineWidth: config.grosorBordeNodo || (tema ? 1.7 : 2),
            font: config.fontNodo || "14px Arial"
