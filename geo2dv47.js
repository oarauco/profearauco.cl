...
(function (window, document) {
  ...
  function getPolygonPartLabel(raw, part) {
    ...
    if (part.kind === 'polygon-fill') return `${raw.id} área`;
    ...
    refreshObjectList() {
      ...
      for (const obj of this.model.objects) {
        ...
        if (InternalObjectAdapter.type(obj) === 'polygon') {
          normalizePolygonParts(raw);
          buckets.get('figures').entries.push({
            objectId: obj.id,
            part: { kind: 'polygon-fill' },
            label: getPolygonPartLabel(raw, { kind: 'polygon-fill' }),
            visible: raw.visible !== false && isPolygonFillVisible(raw)
          });
