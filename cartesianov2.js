...
(function () {
  ...
  function worldToScreen(vp, w, h, x, y) {
    return {
      x: ((x - vp.xMin) / (vp.xMax - vp.xMin)) * w,
      y: h - ((y - vp.yMin) / (vp.yMax - vp.yMin)) * h
    };
  }

  function screenToWorld(vp, w, h, sx, sy) {
    return {
      x: vp.xMin + (sx / w) * (vp.xMax - vp.xMin),
      y: vp.yMin + ((h - sy) / h) * (vp.yMax - vp.yMin)
    };
  }
  ...
  function renderSceneToSvg(svg, scene, state) {
    ...
    for (const { object: obj, resolved } of resolver.allResolved()) {
      ...
      if (resolved.kind === 'circle') {
        ...
        const e = worldToScreen(vp, width, height, resolved.center.x + resolved.radius, resolved.center.y);
    ...
    if (state._pendingPoints.length > 0 && previewAnchor) {
      ...
      if (first && ['segment', 'line', 'circle', 'midpoint', 'measure-distance'].includes(state.activeTool)) {
        ...
        if (state.activeTool === 'circle') {
          ...
          const c = worldToScreen(vp, width, height, first.x, first.y);
          const e = worldToScreen(vp, width, height, first.x + radius, first.y);
...
<\/script>`;
    ...
    /* =========================================================
       PUNTO UNIFICADO
       - Si existe uno cerca, se reutiliza.
    ...
    handleConstructionClick(world, sx, sy) {
      ...
      if (this.activeTool === 'polygon') {
        if (this._pendingPoints[this._pendingPoints.length - 1] !== id) {
    ...
    findNearestNonPointObjectAtScreen(sx, sy) {
      ...
      for (const obj of this.scene.objects) {
        ...
        if (r.kind === 'segment') d2 = this.p2s(sx, sy, r.p1, r.p2, vp);
        if (r.kind === 'line') d2 = this.p2l(sx, sy, r.p1, r.p2, vp);

        if (r.kind === 'circle') {
          const c = worldToScreen(vp, this._svgWidth, this._svgHeight, r.center.x, r.center.y);
          const e = worldToScreen(vp, this._svgWidth, this._svgHeight, r.center.x + r.radius, r.center.y);
          d2 = Math.pow(Math.sqrt(dist2(sx, sy, c.x, c.y)) - Math.abs(e.x - c.x), 2);
        }
