...
if (vp.showGrid) {
  ...
  const baseStep = vp.lockAspect
    ? niceStep(Math.min(spanX, spanY))
    : null;
...
this.svg.addEventListener('wheel', e => {
  ...
  const drawRect = getDrawableRect(this.scene.viewport, w, h);

  const sx = clamp(sxRaw, drawRect.x, drawRect.x + drawRect.width);
  const sy = clamp(syRaw, drawRect.y, drawRect.y + drawRect.height);

  const mw = screenToWorld(this.scene.viewport, w, h, sx, sy);

  const delta = clamp(e.deltaY, -80, 80);
  const factor = Math.exp(delta * 0.0015);

  this.scene.viewport = viewportZoom(this.scene.viewport, factor, mw.x, mw.y);
  this.renderAndSync();
}, { passive: false });
