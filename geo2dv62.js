...
.geo2d-notable-visible {
  ...
  function isHitTestIntersectableResolvedKind(kind) {
    return isHitTestDirectionalResolvedKind(kind) || isCurveResolvedKind(kind) || isCircularBoundaryResolvedKind(kind);
  }
  ...
  function isCircularBoundaryResolvedKind(kind) {
    return kind === 'circle-arc' || kind === 'circular-sector';
  }

  function circularArcResolvedToCircle(resolved) {
    if (!resolved || !isCircularBoundaryResolvedKind(resolved.kind)) return null;
    return {
      kind: 'circle',
  ...
  function circularArcResolvedWorldPoints(resolved, steps = 144) {
    if (!resolved || !isCircularBoundaryResolvedKind(resolved.kind)) return [];
    return angleArcWorldPoints(resolved.center, resolved.radius, resolved.startAngle, resolved.delta, steps);
  }

  function circularSectorRadialSegments(resolved) {
    if (!resolved || resolved.kind !== 'circular-sector') return [];
    const rawSegments = [
      { p1: resolved.center, p2: resolved.start },
      { p1: resolved.center, p2: resolved.endOnCircle }
    ];

    const segments = [];
    for (const segment of rawSegments) {
      if (!segment.p1 || !segment.p2) continue;
      if (dist2(segment.p1.x, segment.p1.y, segment.p2.x, segment.p2.y) < 1e-18) continue;
      if (segments.some(existing => (
        dist2(existing.p1.x, existing.p1.y, segment.p1.x, segment.p1.y) < 1e-18 &&
        dist2(existing.p2.x, existing.p2.y, segment.p2.x, segment.p2.y) < 1e-18
      ))) continue;
      segments.push({
        kind: 'segment',
        p1: segment.p1,
  ...
  function isPointOnCircularArcSpan(resolved, point, tol = 1e-7) {
    if (!resolved || !point || !isCircularBoundaryResolvedKind(resolved.kind)) return false;
    const radius = safeNumber(resolved.radius, NaN);
    if (!(radius > 1e-9)) return false;

    const radialTol = Math.max(tol, radius * 1e-7);
    const distanceFromCenter = dist(resolved.center.x, resolved.center.y, point.x, point.y);
    if (Math.abs(distanceFromCenter - radius) > radialTol) return false;

    const angle = Math.atan2(point.y - resolved.center.y, point.x - resolved.center.x);
    const direction = normalizeAngleDirection(resolved.direction, 'ccw');
    const relative = Math.abs(circularArcDelta(resolved.startAngle, angle, direction));
    const span = Math.abs(safeNumber(resolved.delta, 0));
    const angleTol = Math.max(1e-7, radialTol / radius);
    return relative <= span + angleTol || Math.abs(relative - span) <= angleTol;
  }

  function filterCircularArcHits(resolved, hits) {
    return dedupeWorldPoints((hits || []).filter(point => isPointOnCircularArcSpan(resolved, point)));
  }
  ...
  function polylineIntersections(pointsA, pointsB, closedA = false, closedB = false) {
    const hits = [];
    if (!pointsA || !pointsB || pointsA.length < 2 || pointsB.length < 2) return hits;

    const limitA = closedA ? pointsA.length : pointsA.length - 1;
    const limitB = closedB ? pointsB.length : pointsB.length - 1;
  ...
  function closedPolylineIntersections(pointsA, pointsB) {
    return polylineIntersections(pointsA, pointsB, true, true);
  }
  ...
  function linearCircularBoundaryIntersections(lineResolved, circularResolved) {
    if (!lineResolved || !isLinearResolvedKind(lineResolved.kind) || !isCircularBoundaryResolvedKind(circularResolved?.kind)) return [];

    const circle = circularArcResolvedToCircle(circularResolved);
    const hits = filterCircularArcHits(circularResolved, linearCircleIntersections(lineResolved, circle));

    if (circularResolved.kind === 'circular-sector') {
      for (const segment of circularSectorRadialSegments(circularResolved)) {
        hits.push(...linearLinearIntersections(lineResolved, segment));
      }
    }
  ...
  function circleCircularBoundaryIntersections(circleResolved, circularResolved) {
    if (!circleResolved || circleResolved.kind !== 'circle' || !isCircularBoundaryResolvedKind(circularResolved?.kind)) return [];

    const hits = filterCircularArcHits(
      circularResolved,
      circleCircleIntersections(circleResolved, circularArcResolvedToCircle(circularResolved))
    );

    if (circularResolved.kind === 'circular-sector') {
      for (const segment of circularSectorRadialSegments(circularResolved)) {
        hits.push(...linearCircleIntersections(segment, circleResolved));
      }
    }
  ...
  function ellipseCircularBoundaryIntersections(ellipseResolved, circularResolved) {
    if (!ellipseResolved || ellipseResolved.kind !== 'ellipse' || !isCircularBoundaryResolvedKind(circularResolved?.kind)) return [];

    const arcPoints = circularArcResolvedWorldPoints(circularResolved, 240);
    const ellipsePoints = ellipseWorldPointsFromResolved(ellipseResolved, 240);
    const hits = polylineIntersections(arcPoints, ellipsePoints, false, true);

    if (circularResolved.kind === 'circular-sector') {
      for (const segment of circularSectorRadialSegments(circularResolved)) {
        hits.push(...linearEllipseIntersections(segment, ellipseResolved));
      }
    }
  ...
  function segmentChainCircularBoundaryIntersections(chainResolved, circularResolved, edgeIndex = null) {
    if (!isSegmentChainResolvedKind(chainResolved?.kind) || !isCircularBoundaryResolvedKind(circularResolved?.kind)) return [];
    const normalizedEdgeIndex = normalizeEdgeIndex(edgeIndex);
    const segments = normalizedEdgeIndex === null
      ? resolvedToBoundarySegments(chainResolved)
      : [resolveSegmentLikeReference(chainResolved, normalizedEdgeIndex)].filter(Boolean);

    const hits = [];
    for (const segment of segments) {
      hits.push(...linearCircularBoundaryIntersections(segment, circularResolved));
    }

    return dedupeWorldPoints(hits);
  }

  function circularCircularBoundaryIntersections(resolvedA, resolvedB) {
    if (!isCircularBoundaryResolvedKind(resolvedA?.kind) || !isCircularBoundaryResolvedKind(resolvedB?.kind)) return [];

    const arcHits = circleCircleIntersections(
      circularArcResolvedToCircle(resolvedA),
      circularArcResolvedToCircle(resolvedB)
    ).filter(point => isPointOnCircularArcSpan(resolvedA, point) && isPointOnCircularArcSpan(resolvedB, point));

    const hits = [...arcHits];

    if (resolvedA.kind === 'circular-sector') {
      for (const segment of circularSectorRadialSegments(resolvedA)) {
        hits.push(...linearCircularBoundaryIntersections(segment, resolvedB));
      }
    }

    if (resolvedB.kind === 'circular-sector') {
      for (const segment of circularSectorRadialSegments(resolvedB)) {
        hits.push(...linearCircularBoundaryIntersections(segment, resolvedA));
      }
    }
  ...
  function resolveCircularBoundaryFamilyIntersections(resolvedA, resolvedB, edgeIndexA, edgeIndexB) {
    if (isLinearResolvedKind(resolvedA.kind) && isCircularBoundaryResolvedKind(resolvedB.kind)) {
      return linearCircularBoundaryIntersections(resolvedA, resolvedB);
    }

    if (isLinearResolvedKind(resolvedB.kind) && isCircularBoundaryResolvedKind(resolvedA.kind)) {
      return linearCircularBoundaryIntersections(resolvedB, resolvedA);
    }

    if (resolvedA.kind === 'circle' && isCircularBoundaryResolvedKind(resolvedB.kind)) {
      return circleCircularBoundaryIntersections(resolvedA, resolvedB);
    }

    if (resolvedB.kind === 'circle' && isCircularBoundaryResolvedKind(resolvedA.kind)) {
      return circleCircularBoundaryIntersections(resolvedB, resolvedA);
    }

    if (resolvedA.kind === 'ellipse' && isCircularBoundaryResolvedKind(resolvedB.kind)) {
      return ellipseCircularBoundaryIntersections(resolvedA, resolvedB);
    }

    if (resolvedB.kind === 'ellipse' && isCircularBoundaryResolvedKind(resolvedA.kind)) {
      return ellipseCircularBoundaryIntersections(resolvedB, resolvedA);
    }

    if (isSegmentChainResolvedKind(resolvedA.kind) && isCircularBoundaryResolvedKind(resolvedB.kind)) {
      return segmentChainCircularBoundaryIntersections(resolvedA, resolvedB, edgeIndexA);
    }

    if (isSegmentChainResolvedKind(resolvedB.kind) && isCircularBoundaryResolvedKind(resolvedA.kind)) {
      return segmentChainCircularBoundaryIntersections(resolvedB, resolvedA, edgeIndexB);
    }

    if (isCircularBoundaryResolvedKind(resolvedA.kind) && isCircularBoundaryResolvedKind(resolvedB.kind)) {
      return circularCircularBoundaryIntersections(resolvedA, resolvedB);
    }
  ...
  const INTERSECTION_RESOLVERS = Object.freeze([
    ...
    context => resolveCircularBoundaryFamilyIntersections(
      context.resolvedA,
      context.resolvedB,
      context.edgeIndexA,
      context.edgeIndexB
  ...
  class Geo2DHitTester {
    ...
    getObjectBoundaryHit(sx, sy, resolved, vp) {
      ...
      if (resolved.kind === 'circle-arc' || resolved.kind === 'circular-sector') {
        ...
        if (resolved.kind === 'circle-arc') return arcHit;

        let best = arcHit;
        for (const segment of circularSectorRadialSegments(resolved)) {
          const d2 = this.p2s(sx, sy, segment.p1, segment.p2, vp);
          if (d2 < best.d2) {
            best = {
              d2,
              edgeIndex: null,
