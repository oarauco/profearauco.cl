(function (window, document) {
  "use strict";

  const MENSAJE_MOTOR_FALTANTE =
    "grafiestadistik-autov3.js: no se encontro window.GrafiEstadistik completo. Carga antes grafiestadistik-motorv2.js";

  function obtenerAPI() {
    return window.GrafiEstadistik || null;
  }

  function motorCompletoDisponible(api) {
    return !!(
      api &&
      typeof api.dibujarGraficoBarra === "function" &&
      typeof api.dibujarGraficoBarrasMultiples === "function" &&
      typeof api.dibujarGraficoBarrasDobles === "function" &&
      typeof api.dibujarPictograma === "function" &&
      typeof api.dibujarHistograma === "function" &&
      typeof api.dibujarGraficoCircular === "function" &&
      typeof api.dibujarGraficoCaja === "function" &&
      typeof api.dibujarOjiva === "function" &&
      typeof api.dibujarGraficoDispersion === "function" &&
      typeof api.dibujarGraficoDispersionRegresion === "function" &&
      typeof api.dibujarGraficoLineas === "function" &&
      typeof api.dibujarPoligonoFrecuencia === "function" &&
      typeof api.dibujarDistribucionBinomial === "function" &&
      typeof api.dibujarDistribucionNormal === "function" &&
      typeof api.dibujarIntervaloConfianza === "function" &&
      typeof api.dibujarPruebaHipotesis === "function"
    );
  }

  function normalizarRoot(root) {
    if (root && typeof root.querySelectorAll === "function") return root;
    return document;
  }

  function seleccionarTodos(root, selector) {
    return Array.from(normalizarRoot(root).querySelectorAll(selector));
  }

  function parsearJSONAttr(el, nombre, fallback) {
    const raw = el.dataset[nombre];
    if (raw === undefined || raw === null || raw === "") return fallback;

    try {
      return JSON.parse(raw);
    } catch (error) {
      console.error(`Error al parsear data-${nombre}:`, error, el);
      return fallback;
    }
  }

  function parsearNumeroAttr(el, nombre, fallback = null) {
    const raw = el.dataset[nombre];
    if (raw === undefined || raw === null || raw === "") return fallback;

    const valor = parseFloat(raw);
    return Number.isFinite(valor) ? valor : fallback;
  }

  function parsearEnteroAttr(el, nombre, fallback = null) {
    const raw = el.dataset[nombre];
    if (raw === undefined || raw === null || raw === "") return fallback;

    const valor = parseInt(raw, 10);
    return Number.isInteger(valor) ? valor : fallback;
  }

  function parsearRangoAttr(el, nombre, fallback = null) {
    const raw = el.dataset[nombre];
    if (raw === undefined || raw === null || raw === "") return fallback;

    const partes = String(raw).split(",").map((v) => parseFloat(v.trim()));
    if (partes.length !== 2) return fallback;
    if (!Number.isFinite(partes[0]) || !Number.isFinite(partes[1])) return fallback;

    return {
      min: partes[0],
      max: partes[1]
    };
  }

  function procesarColeccion(root, selector, etiquetaError, callback) {
    seleccionarTodos(root, selector).forEach(function (el) {
      try {
        callback(el);
      } catch (error) {
        console.error(`${etiquetaError}:`, error, el);
      }
    });
  }

  function inicializarGraficosBarra(root, api) {
    procesarColeccion(root, ".grafico-barra-auto", "Error en gráfico de barras", function (div) {
      const id = div.id;
      const frecuencias = parsearJSONAttr(div, "frecuencias", []);
      const etiquetasX = parsearJSONAttr(div, "etiquetas", []);
      const color = div.dataset.color || "#ccc";
      const minY = parsearNumeroAttr(div, "minY", null);
      const maxY = parsearNumeroAttr(div, "maxY", null);
      const ticksY = parsearEnteroAttr(div, "ticksY", null);
      const posicionEtiquetas = parsearEnteroAttr(div, "posicionEtiquetas", 1);

      if (
        id &&
        Array.isArray(frecuencias) &&
        Array.isArray(etiquetasX) &&
        frecuencias.length === etiquetasX.length &&
        frecuencias.length > 0
      ) {
        api.dibujarGraficoBarra(
          id,
          frecuencias,
          etiquetasX,
          color,
          minY,
          maxY,
          ticksY,
          posicionEtiquetas
        );
      }
    });
  }

  function inicializarGraficosBarrasDobles(root, api) {
    procesarColeccion(root, ".grafico-barras-dobles-auto", "Error en grafico de barras dobles", function (div) {
      const id = div.id;
      const serieA = parsearJSONAttr(div, "serieA", []);
      const serieB = parsearJSONAttr(div, "serieB", []);
      const etiquetasX = parsearJSONAttr(div, "etiquetas", []);

      const opciones = {
        nombreA: div.dataset.nombreA || div.dataset.etiquetaA || "Serie A",
        nombreB: div.dataset.nombreB || div.dataset.etiquetaB || "Serie B",
        colorA: div.dataset.colorA || "#356f9f",
        colorB: div.dataset.colorB || "#c95056",
        minY: parsearNumeroAttr(div, "minY", null),
        maxY: parsearNumeroAttr(div, "maxY", null),
        ticksY: parsearEnteroAttr(div, "ticksY", null),
        posicionEtiquetas: parsearEnteroAttr(div, "posicionEtiquetas", 1)
      };

      if (
        id &&
        Array.isArray(serieA) &&
        Array.isArray(serieB) &&
        Array.isArray(etiquetasX) &&
        serieA.length === serieB.length &&
        serieA.length === etiquetasX.length &&
        serieA.length > 0
      ) {
        api.dibujarGraficoBarrasDobles(id, serieA, serieB, etiquetasX, opciones);
      }
    });
  }

  function inicializarGraficosBarrasMultiples(root, api) {
    procesarColeccion(root, ".grafico-barras-multiples-auto", "Error en grafico de barras multiples", function (div) {
      const id = div.id;
      const series = parsearJSONAttr(div, "series", []);
      const etiquetasX = parsearJSONAttr(div, "etiquetas", []);

      const opciones = {
        minY: parsearNumeroAttr(div, "minY", null),
        maxY: parsearNumeroAttr(div, "maxY", null),
        ticksY: parsearEnteroAttr(div, "ticksY", null),
        posicionEtiquetas: parsearEnteroAttr(div, "posicionEtiquetas", 1)
      };

      const seriesValidas = Array.isArray(series)
        ? series.filter((serie) => serie && Array.isArray(serie.valores))
        : [];

      if (
        id &&
        Array.isArray(etiquetasX) &&
        etiquetasX.length > 0 &&
        seriesValidas.length >= 2 &&
        seriesValidas.every((serie) => serie.valores.length === etiquetasX.length)
      ) {
        api.dibujarGraficoBarrasMultiples(id, seriesValidas, etiquetasX, opciones);
      }
    });
  }

  function inicializarPictogramas(root, api) {
    procesarColeccion(root, ".pictograma-auto", "Error en pictograma", function (div) {
      const id = div.id;
      const datos = parsearJSONAttr(div, "datos", []);
      const icono = div.dataset.icono || "";
      const valorPorIcono = parsearNumeroAttr(div, "valorPorIcono", null);
      const color = div.dataset.color || "#666";

      if (
        id &&
        Array.isArray(datos) &&
        datos.length > 0 &&
        icono &&
        Number.isFinite(valorPorIcono) &&
        valorPorIcono > 0
      ) {
        api.dibujarPictograma(id, datos, icono, valorPorIcono, color);
      }
    });
  }

  function inicializarHistogramas(root, api) {
    procesarColeccion(root, ".histograma-auto", "Error en histograma", function (div) {
      const id = div.id;
      const frecuencias = parsearJSONAttr(div, "frecuencias", []);
      const limites = parsearJSONAttr(div, "limites", []);
      const color = div.dataset.color || "#ccc";
      const minY = parsearNumeroAttr(div, "minY", null);
      const maxY = parsearNumeroAttr(div, "maxY", null);
      const ticksY = parsearEnteroAttr(div, "ticksY", null);

      if (
        id &&
        Array.isArray(frecuencias) &&
        Array.isArray(limites) &&
        frecuencias.length > 0 &&
        limites.length === frecuencias.length + 1
      ) {
        api.dibujarHistograma(id, frecuencias, limites, color, minY, maxY, ticksY);
      }
    });
  }

  function inicializarGraficosCirculares(root, api) {
    procesarColeccion(root, ".grafico-circular-auto", "Error en gráfico circular", function (div) {
      const id = div.id;
      const datos = parsearJSONAttr(div, "datos", []);

      if (id && Array.isArray(datos) && datos.length > 0) {
        api.dibujarGraficoCircular(id, datos);
      }
    });
  }

  function inicializarGraficosCaja(root, api) {
    procesarColeccion(root, ".caja-auto", "Error en gráfico de caja", function (div) {
      const id = div.id;
      const valores = parsearJSONAttr(div, "valores", null);
      const color = div.dataset.color || "rgba(255, 159, 64, 0.5)";

      if (id && valores && typeof valores === "object") {
        api.dibujarGraficoCaja(id, valores, color);
      }
    });
  }

  function inicializarOjivas(root, api) {
    procesarColeccion(root, ".ojiva-auto", "Error en ojiva", function (div) {
      const id = div.id;
      const limites = parsearJSONAttr(div, "limites", []);
      const frecuenciasAcumuladas = parsearJSONAttr(div, "frecuenciasAcumuladas", []);
      const color = div.dataset.color || "#4bc0c0";
      const minY = parsearNumeroAttr(div, "minY", null);
      const maxY = parsearNumeroAttr(div, "maxY", null);
      const ticksY = parsearEnteroAttr(div, "ticksY", null);

      let etiquetasEjes = parsearJSONAttr(div, "etiquetasEjes", {});
      etiquetasEjes = {
        x: etiquetasEjes.x || div.dataset.etiquetaX || "Valor observado",
        y: etiquetasEjes.y || div.dataset.etiquetaY || "Frecuencia acumulada"
      };

      if (
        id &&
        Array.isArray(limites) &&
        Array.isArray(frecuenciasAcumuladas) &&
        limites.length > 0 &&
        frecuenciasAcumuladas.length > 0
      ) {
        api.dibujarOjiva(
          id,
          limites,
          frecuenciasAcumuladas,
          color,
          minY,
          maxY,
          ticksY,
          etiquetasEjes
        );
      }
    });
  }

  function inicializarGraficosDispersion(root, api) {
    procesarColeccion(root, ".grafico-dispersion-auto", "Error en grafico de dispersion", function (div) {
      const id = div.id;
      const puntos = parsearJSONAttr(div, "puntos", []);

      let etiquetas = parsearJSONAttr(div, "etiquetas", {});
      const etiquetasEjes = parsearJSONAttr(div, "etiquetasEjes", {});

      etiquetas = {
        ...etiquetas,
        x: etiquetas.x || etiquetasEjes.x || div.dataset.etiquetaX || "X",
        y: etiquetas.y || etiquetasEjes.y || div.dataset.etiquetaY || "Y"
      };

      const rangoX = parsearRangoAttr(div, "rangeX", null);
      const rangoY = parsearRangoAttr(div, "rangeY", null);

      if (rangoX) {
        etiquetas.minX = rangoX.min;
        etiquetas.maxX = rangoX.max;
      }

      if (rangoY) {
        etiquetas.minY = rangoY.min;
        etiquetas.maxY = rangoY.max;
      }

      const stepX = parsearNumeroAttr(div, "stepX", null);
      const stepY = parsearNumeroAttr(div, "stepY", null);

      if (Number.isFinite(stepX) && stepX > 0) {
        etiquetas.stepX = stepX;
      } else {
        etiquetas.ticksX = parsearEnteroAttr(div, "ticksX", etiquetas.ticksX || null);
      }

      if (Number.isFinite(stepY) && stepY > 0) {
        etiquetas.stepY = stepY;
      } else {
        etiquetas.ticksY = parsearEnteroAttr(div, "ticksY", etiquetas.ticksY || null);
      }

      etiquetas.anchoSVG = parsearNumeroAttr(div, "width", null);
      etiquetas.altoSVG = parsearNumeroAttr(div, "height", null);

      const radioPunto = parsearNumeroAttr(div, "radioPunto", null);
      if (Number.isFinite(radioPunto) && radioPunto > 0) {
        etiquetas.radioPunto = radioPunto;
      }

      if (div.dataset.regresion === "true") {
        etiquetas.mostrarRegresion = true;
        etiquetas.colorRegresion = div.dataset.colorRegresion || etiquetas.colorRegresion;
      }

      const color = div.dataset.color || "steelblue";

      if (id && Array.isArray(puntos) && puntos.length > 0) {
        api.dibujarGraficoDispersion(id, puntos, etiquetas, color);
      }
    });
  }

  function inicializarGraficosLineas(root, api) {
    procesarColeccion(root, ".grafico-lineas-auto", "Error en gráfico de líneas", function (div) {
      const id = div.id;
      const etiquetasX = parsearJSONAttr(div, "etiquetasX", []);
      const valoresY = parsearJSONAttr(div, "valoresY", []);
      const etiquetasEjes = parsearJSONAttr(div, "etiquetasEjes", {});
      const color = div.dataset.color || "#3e95cd";

      if (
        id &&
        Array.isArray(etiquetasX) &&
        Array.isArray(valoresY) &&
        etiquetasX.length === valoresY.length &&
        etiquetasX.length > 0
      ) {
        api.dibujarGraficoLineas(id, etiquetasX, valoresY, etiquetasEjes, color);
      }
    });
  }

  function inicializarPoligonosFrecuencia(root, api) {
    procesarColeccion(root, ".poligono-frecuencia-auto", "Error en poligono de frecuencia", function (div) {
      const id = div.id;
      const marcasClase = parsearJSONAttr(div, "marcasClase", parsearJSONAttr(div, "etiquetasX", []));
      const frecuencias = parsearJSONAttr(div, "frecuencias", []);
      const etiquetasEjes = parsearJSONAttr(div, "etiquetasEjes", {});
      const color = div.dataset.color || "#356f9f";

      if (
        id &&
        Array.isArray(marcasClase) &&
        Array.isArray(frecuencias) &&
        marcasClase.length === frecuencias.length &&
        marcasClase.length > 0
      ) {
        api.dibujarPoligonoFrecuencia(id, marcasClase, frecuencias, etiquetasEjes, color);
      }
    });
  }

  function inicializarDistribucionesBinomiales(root, api) {
    procesarColeccion(root, ".binomial-auto", "Error en distribucion binomial", function (div) {
      const id = div.id;
      const n = parsearEnteroAttr(div, "n", null);
      const p = parsearNumeroAttr(div, "p", null);
      const opciones = {
        color: div.dataset.color,
        colorDestacado: div.dataset.colorDestacado,
        destacarDesde: parsearNumeroAttr(div, "destacarDesde", null),
        destacarHasta: parsearNumeroAttr(div, "destacarHasta", null)
      };

      if (id && Number.isInteger(n) && Number.isFinite(p)) {
        api.dibujarDistribucionBinomial(id, n, p, opciones);
      }
    });
  }

  function inicializarDistribucionesNormales(root, api) {
    procesarColeccion(root, ".normal-auto", "Error en distribucion normal", function (div) {
      const id = div.id;
      const media = parsearNumeroAttr(div, "media", 0);
      const desviacion = parsearNumeroAttr(div, "desviacion", 1);
      const opciones = {
        minX: parsearNumeroAttr(div, "minX", null),
        maxX: parsearNumeroAttr(div, "maxX", null),
        sombrearDesde: parsearNumeroAttr(div, "sombrearDesde", null),
        sombrearHasta: parsearNumeroAttr(div, "sombrearHasta", null),
        color: div.dataset.color,
        colorArea: div.dataset.colorArea,
        ejeX: div.dataset.ejeX
      };

      if (id && Number.isFinite(media) && Number.isFinite(desviacion)) {
        api.dibujarDistribucionNormal(id, media, desviacion, opciones);
      }
    });
  }

  function inicializarIntervalosConfianza(root, api) {
    procesarColeccion(root, ".intervalo-confianza-auto", "Error en intervalo de confianza", function (div) {
      const id = div.id;
      const estimacion = parsearNumeroAttr(div, "estimacion", null);
      const limiteInferior = parsearNumeroAttr(div, "limiteInferior", null);
      const limiteSuperior = parsearNumeroAttr(div, "limiteSuperior", null);
      const opciones = {
        minX: parsearNumeroAttr(div, "minX", null),
        maxX: parsearNumeroAttr(div, "maxX", null),
        color: div.dataset.color,
        titulo: div.dataset.titulo
      };

      if (
        id &&
        Number.isFinite(estimacion) &&
        Number.isFinite(limiteInferior) &&
        Number.isFinite(limiteSuperior)
      ) {
        api.dibujarIntervaloConfianza(id, estimacion, limiteInferior, limiteSuperior, opciones);
      }
    });
  }

  function inicializarPruebasHipotesis(root, api) {
    procesarColeccion(root, ".prueba-hipotesis-auto", "Error en prueba de hipotesis", function (div) {
      const id = div.id;
      const opciones = {
        tipo: div.dataset.tipo || "bilateral",
        media: parsearNumeroAttr(div, "media", 0),
        desviacion: parsearNumeroAttr(div, "desviacion", 1),
        critico: parsearNumeroAttr(div, "critico", 1.96),
        estadistico: parsearNumeroAttr(div, "estadistico", null),
        color: div.dataset.color,
        colorArea: div.dataset.colorArea,
        ejeX: div.dataset.ejeX
      };

      if (id) {
        api.dibujarPruebaHipotesis(id, opciones);
      }
    });
  }

  function inicializar(root = document) {
    const rootReal = normalizarRoot(root);
    const api = obtenerAPI();

    if (!motorCompletoDisponible(api)) {
      console.error(MENSAJE_MOTOR_FALTANTE);
      return false;
    }

    inicializarGraficosBarra(rootReal, api);
    inicializarGraficosBarrasMultiples(rootReal, api);
    inicializarGraficosBarrasDobles(rootReal, api);
    inicializarPictogramas(rootReal, api);
    inicializarHistogramas(rootReal, api);
    inicializarGraficosCirculares(rootReal, api);
    inicializarGraficosCaja(rootReal, api);
    inicializarOjivas(rootReal, api);
    inicializarGraficosDispersion(rootReal, api);
    inicializarGraficosLineas(rootReal, api);
    inicializarPoligonosFrecuencia(rootReal, api);
    inicializarDistribucionesBinomiales(rootReal, api);
    inicializarDistribucionesNormales(rootReal, api);
    inicializarIntervalosConfianza(rootReal, api);
    inicializarPruebasHipotesis(rootReal, api);

    return true;
  }

  function autoInicializar() {
    inicializar(document);
  }

  if (document.readyState === "loading") {
    document.addEventListener("DOMContentLoaded", autoInicializar);
  } else {
    autoInicializar();
  }

  window.GrafiEstadistikAuto = {
    inicializar: inicializar,
    autoInicializar: autoInicializar,
    obtenerAPI: obtenerAPI
  };
})(window, document);
