...
(function (window, document) {
  ...
  function validateAreaObject(raw, validationContext) {
    validatePointSequenceObject(raw, validationContext, 3, value => `El área "${value.id}" debe tener al menos 3 puntos.`);
  }
...
.geo2d-check {
  ...
  const INTERNAL_OBJECT_REGISTRY = Object.freeze({
    ...
    area: Object.freeze({
      create: raw => new AreaObject(raw),
      toConstruction: raw => buildPointSequenceConstruction(raw, 'area'),
      validate: validateAreaObject
    }),
  ...
  function createInternalObjectCreators(registry) {
    const creators = {};
    for (const [type, entry] of Object.entries(registry)) {
      creators[type] = entry.create;
    }
    return Object.freeze(creators);
  }

  const SUPPORTED_INTERNAL_OBJECT_TYPES = new Set(Object.keys(INTERNAL_OBJECT_REGISTRY));
  const INTERNAL_OBJECT_CREATORS = createInternalObjectCreators(INTERNAL_OBJECT_REGISTRY);

  class GeoFactory {
    static fromRaw(raw) {
      if (!raw || typeof raw !== 'object') return new UnknownObject({});

      const createObject = INTERNAL_OBJECT_CREATORS[InternalObjectAdapter.type(raw)];
