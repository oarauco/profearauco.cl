...
.geo2d-notable-visible {
  ...
  function resolveEllipseGeometryFromPoints(center, vertex, coVertex) {
    ...
    const rx = dist(center.x, center.y, vertex.x, vertex.y);
    if (!(rx > 1e-9)) return null;

    const rotation = Math.atan2(vertex.y - center.y, vertex.x - center.x);
    const localCoVertex = ellipseLocalCoordinates(center, rotation, coVertex.x, coVertex.y);
    const signedRy = localCoVertex.y;
    const ry = Math.abs(signedRy);
    if (!(ry > 1e-9)) return null;

    const cosR = Math.cos(rotation);
    const sinR = Math.sin(rotation);
    const projectedCoVertex = {
      x: center.x - signedRy * sinR,
      y: center.y + signedRy * cosR
    };
    const antiVertex = {
    ...
    const antiCoVertex = {
      x: center.x * 2 - projectedCoVertex.x,
      y: center.y * 2 - projectedCoVertex.y
    };

    return {
      center,
      vertex,
      coVertex: projectedCoVertex,
      controlCoVertex: coVertex,
      antiVertex,
      antiCoVertex,
      rx,
      ry,
  ...
  function renderEllipsePreview(ctx, first, second) {
    ...
    const screenPts = ellipseWorldPoints(first, draft.rx, draft.ry, draft.rotation, 96)
      .map(p => worldToScreen(vp, width, height, p.x, p.y));
