...
(function () {
  ...
  function drawFunctionFromExpr(svg, expr, map, xmin, xmax, ymin, ymax, color, doClip) {
    ...
    for (let x = xmin; x <= xmax + 1e-12; x += step) {
      ...
      if (!doClip) {
        if (yPrev !== null && screenJump(y, yPrev) > GRAPH_SETTINGS.noClipJumpFactor * map.h) {
          flush();
        }
        seg.push([x, y]);
        xPrev = x;
      ...
      if (inPrev && inNow) {
        if (shouldForceBreak(yPrev, y)) {
          ...
          if (screenJump(y, yPrev) > GRAPH_SETTINGS.visibleJumpFactor * map.h) {
            flush();
          }
          seg.push([x, y]);
        }
