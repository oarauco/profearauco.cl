...
const MATHIO_CSS = `
        ...
        .mathio-juego, .mathio-juego * { -webkit-text-size-adjust: 100%; text-size-adjust: 100%; }
        ...
        .mathio-juego .mathio-ui-container { position: absolute; top: 10px; left: 50%; transform: translateX(-50%); width: min(70%, 460px); text-align: center; pointer-events: none; z-index: 10; display: flex; flex-direction: column; align-items: center; gap: 5px; }
        .mathio-juego .mathio-score-display { position: relative; width: min(260px, 76vw); height: 12px; overflow: hidden; border-radius: 999px; background: rgba(255,255,255,0.14); border: 1px solid rgba(255,255,255,0.18); box-shadow: 0 2px 8px rgba(0,0,0,0.35); font-size: 0; line-height: 0; font-weight: 800; color: rgba(255,255,255,0.88); text-shadow: 0 1px 2px rgba(0,0,0,0.85); }
        ...
        .mathio-juego .mathio-score-display span { position: absolute; inset: 0; z-index: 1; display: flex; align-items: center; justify-content: center; font-size: 7px !important; line-height: 1 !important; font-family: Arial, sans-serif !important; font-weight: 800; transform: scale(0.9); transform-origin: center; }
        .mathio-juego .mathio-score-display { order: 2; }
        .mathio-juego .mathio-target-expression { order: 1; font-size: clamp(15px, 2.3vw, 20px) !important; background: rgba(0,0,0,0.72); padding: 5px 14px; border-radius: 12px; display: inline-block; font-weight: bold; box-shadow: 0 0 10px rgba(255,255,255,0.09); letter-spacing: 0; max-width: 100%; line-height: 1.15; }
        .mathio-juego .mathio-target-expression .katex { font-size: 1em !important; }
        .mathio-juego .mathio-target-expression .katex-display { margin: 0; }
        ...
        .mathio-juego .mathio-fullscreen-btn { position: absolute; top: 9px; right: 126px; width: 32px; height: 32px; background: rgba(0,0,0,0.34); color: white; border: 2px solid rgba(255,255,255,0.42); border-radius: 9px; font-size: 17px; line-height: 1; cursor: pointer; z-index: 20; display: flex; align-items: center; justify-content: center; transition: all 0.2s ease; backdrop-filter: blur(4px); }
        .mathio-juego .mathio-fullscreen-btn:hover { background: rgba(0,0,0,0.74); border-color: white; transform: scale(1.05); }
        ...
        @media (max-width: 640px) {
            .mathio-juego .mathio-ui-container { top: 42px; width: min(62%, 270px); gap: 4px; }
            .mathio-juego .mathio-target-expression { font-size: clamp(12px, 4vw, 16px) !important; padding: 4px 10px; border-radius: 10px; max-width: 100%; }
            .mathio-juego .mathio-score-display { width: min(190px, 56vw); height: 10px; }
            .mathio-juego .mathio-score-display span { font-size: 6px !important; transform: scale(0.82); }
            .mathio-juego .mathio-fullscreen-btn { top: 8px; right: 96px; width: 28px; height: 28px; border-radius: 8px; font-size: 15px; }
        }
...
const KATEX_CDN_MODULE = 'https://cdn.jsdelivr.net/npm/katex@0.16.11/dist/katex.mjs';
let katexLoaderPromise = null;

function leerConfigKatex(root) {
    const globalConfig = window.MathIO2Katex || window.MathIOKatex || {};
    const desactivado = root?.dataset?.mathioKatex === 'off' || globalConfig.enabled === false;
    if (desactivado) return { css: '', js: '', module: '' };
    const jsExplicito = root?.dataset?.mathioKatexJs || globalConfig.js || globalConfig.katexJs || '';
    const moduloExplicito = root?.dataset?.mathioKatexModule || globalConfig.module || globalConfig.katexModule || '';
    return {
        css: root?.dataset?.mathioKatexCss || globalConfig.css || globalConfig.katexCss || KATEX_CDN_CSS,
        js: jsExplicito,
        module: moduloExplicito || (jsExplicito ? '' : KATEX_CDN_MODULE)
    };
}
...
function montarKatexSiConfigurado(root) {
    ...
    const recursoJS = config.module || config.js;
    if (!recursoJS) return Promise.resolve(false);
    ...
    const cargarConScriptClasico = () => new Promise(resolve => {
        ...
        script.src = config.js || recursoJS;
        script.async = true;
        script.onload = () => {
            const cargado = !!(window.katex && typeof window.katex.renderToString === 'function');
            if (!cargado) katexLoaderPromise = null;
        ...
        script.onerror = () => {
            console.warn('[Math-io] No se pudo cargar KaTeX desde:', script.src);
            katexLoaderPromise = null;
            resolve(false);
        };
    ...
    if (config.module) {
        katexLoaderPromise = import(config.module)
            .then(modulo => {
                window.katex = modulo.default || modulo;
                return !!(window.katex && typeof window.katex.renderToString === 'function');
            })
            .catch(err => {
                console.warn('[Math-io] No se pudo cargar KaTeX como modulo. Se intentara script clasico.', err);
                if (!config.js) return false;
                return cargarConScriptClasico();
            })
            .then(cargado => {
                if (!cargado) katexLoaderPromise = null;
                return cargado;
        ...
        katexLoaderPromise = cargarConScriptClasico();
    }
...
const musicaJuego = {
    habilitada: root.dataset.mathioMusica !== 'off',
    iniciada: false,
    activa: false,
    programada: false,
    paso: 0,
...
function volumenMusica() {
    const valor = Number(root.dataset.mathioMusicaVolumen);
    if (Number.isFinite(valor) && valor >= 0) return Math.min(0.12, valor);
    return 0.045;
}

function frecuenciaMidi(nota) {
    return 440 * Math.pow(2, (nota - 69) / 12);
}

function prepararMusica(ctxAudio) {
    if (musicaJuego.master || !ctxAudio) return;
    musicaJuego.master = ctxAudio.createGain();
    musicaJuego.master.gain.setValueAtTime(0.0001, ctxAudio.currentTime);
    musicaJuego.master.connect(ctxAudio.destination);

    musicaJuego.delay = ctxAudio.createDelay(1.4);
    musicaJuego.delay.delayTime.value = 0.42;
    musicaJuego.feedback = ctxAudio.createGain();
    musicaJuego.feedback.gain.value = 0.32;
    musicaJuego.wet = ctxAudio.createGain();
    musicaJuego.wet.gain.value = 0.18;
    musicaJuego.delay.connect(musicaJuego.feedback);
    musicaJuego.feedback.connect(musicaJuego.delay);
    musicaJuego.delay.connect(musicaJuego.wet);
    musicaJuego.wet.connect(musicaJuego.master);
}

function tocarNotaAcuatica(nota, duracion = 1.4, volumen = 0.055, tipo = 'sine', delay = 0) {
    const ctxAudio = audioCtx;
    if (!ctxAudio || !musicaJuego.master || ctxAudio.state === 'closed') return;
    const ahora = ctxAudio.currentTime + delay;
    const osc = ctxAudio.createOscillator();
    const filtro = ctxAudio.createBiquadFilter();
    const gain = ctxAudio.createGain();

    osc.type = tipo;
    osc.frequency.setValueAtTime(frecuenciaMidi(nota), ahora);
    osc.frequency.exponentialRampToValueAtTime(frecuenciaMidi(nota) * 1.006, ahora + duracion);
    filtro.type = 'lowpass';
    filtro.frequency.setValueAtTime(980, ahora);
    filtro.Q.setValueAtTime(0.65, ahora);
    gain.gain.setValueAtTime(0.0001, ahora);
    gain.gain.exponentialRampToValueAtTime(volumen, ahora + 0.08);
    gain.gain.exponentialRampToValueAtTime(0.0001, ahora + duracion);

    osc.connect(filtro);
    filtro.connect(gain);
    gain.connect(musicaJuego.master);
    if (musicaJuego.delay) gain.connect(musicaJuego.delay);
    osc.start(ahora);
    osc.stop(ahora + duracion + 0.08);
}

function tocarGotaAcuatica() {
    const ctxAudio = audioCtx;
    if (!ctxAudio || !musicaJuego.master || ctxAudio.state === 'closed') return;
    const ahora = ctxAudio.currentTime + 0.02;
    const osc = ctxAudio.createOscillator();
    const gain = ctxAudio.createGain();
    const base = 86 + Math.floor(Math.random() * 10);
    osc.type = 'sine';
    osc.frequency.setValueAtTime(frecuenciaMidi(base), ahora);
    osc.frequency.exponentialRampToValueAtTime(frecuenciaMidi(base + 7), ahora + 0.22);
    gain.gain.setValueAtTime(0.0001, ahora);
    gain.gain.exponentialRampToValueAtTime(0.028, ahora + 0.035);
...
function tocarAcordeAcuatico() {
    ...
    const acorde = acordes[Math.floor(musicaJuego.paso / 16) % acordes.length];
    acorde.forEach((nota, i) => tocarNotaAcuatica(nota, 6.4, i === 0 ? 0.026 : 0.018, i % 2 ? 'triangle' : 'sine', i * 0.05));
}

function programarMusicaAcuatica() {
    if (!musicaJuego.activa || musicaJuego.programada || destroyed) return;
    musicaJuego.programada = true;
    programarTimeout(() => {
        musicaJuego.programada = false;
        if (!musicaJuego.activa || destroyed || (gameState !== 'PLAYING' && gameState !== 'TRANSITION')) return;

        const patron = [50, 57, 62, 69, 64, 62, 57, 55, 52, 59, 64, 67, 62, 59, 57, 50];
        if (musicaJuego.paso % 16 === 0) tocarAcordeAcuatico();
        tocarNotaAcuatica(patron[musicaJuego.paso % patron.length], 1.7, musicaJuego.paso % 4 === 0 ? 0.038 : 0.026, musicaJuego.paso % 2 ? 'triangle' : 'sine');
        if (musicaJuego.paso % 5 === 2) tocarGotaAcuatica();
        musicaJuego.paso++;
        programarMusicaAcuatica();
    }, 620);
}

function iniciarMusicaJuego() {
    if (!musicaJuego.habilitada || destroyed) return;
    const ctxAudio = obtenerAudioContext();
    if (!ctxAudio) return;
    const primeraVez = !musicaJuego.iniciada;
    prepararMusica(ctxAudio);
    musicaJuego.iniciada = true;
    musicaJuego.activa = true;
    const ahora = ctxAudio.currentTime;
    musicaJuego.master.gain.cancelScheduledValues(ahora);
    musicaJuego.master.gain.setValueAtTime(Math.max(0.0001, musicaJuego.master.gain.value || 0.0001), ahora);
    musicaJuego.master.gain.exponentialRampToValueAtTime(volumenMusica(), ahora + 1.2);
    if (primeraVez) {
        tocarAcordeAcuatico();
        tocarNotaAcuatica(62, 1.9, 0.03, 'sine', 0.08);
    }
    programarMusicaAcuatica();
}

function pausarMusicaJuego(fade = 0.8) {
    if (!musicaJuego.master || !audioCtx || audioCtx.state === 'closed') return;
    musicaJuego.activa = false;
    const ahora = audioCtx.currentTime;
    musicaJuego.master.gain.cancelScheduledValues(ahora);
    musicaJuego.master.gain.setValueAtTime(Math.max(0.0001, musicaJuego.master.gain.value || 0.0001), ahora);
    musicaJuego.master.gain.exponentialRampToValueAtTime(0.0001, ahora + fade);
}

function reanudarMusicaSiCorresponde() {
    if (musicaJuego.iniciada && musicaJuego.habilitada) iniciarMusicaJuego();
}
...
function fallar(razon, errX, errY) {
    ...
    lives--;
    if (lives <= 0) {
        gameState = 'GAMEOVER'; pausarMusicaJuego(0.35); playSound('gameover');
...
function dibujarEtiquetaDOM(clave, x, y, fragmentos, radio) {
    ...
    let fontSize = 18; if (textoPlano.length > 18) fontSize = 16; if (textoPlano.length > 30) fontSize = 13;
...
function drawHUD() {
    const compacto = canvas.width < 560;
    ctx.font = `${compacto ? 18 : 24}px Arial`; ctx.textAlign = "left"; ctx.textBaseline = "top";
    ...
    ctx.fillStyle = "white"; ctx.font = `bold ${compacto ? 13 : 17}px 'Segoe UI'`; ctx.textAlign = "right"; ctx.fillText(`Nivel ${nivelVisible} / ${niveles.length}`, canvas.width - (compacto ? 56 : 68), compacto ? 15 : 14);
}
