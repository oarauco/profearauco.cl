...
(function () {
  ...
  class JuegoSnakeMatematico {
    constructor(contenedor, dataset, opciones = {}) {
      ...
      this.recalcularCoordenadasLayout();
    ...
    detectarOrientacionFlipped() {
      if (window.screen && window.screen.orientation) {
        const type = window.screen.orientation.type;
        // landscape-secondary significa que el teléfono está rotado 180 grados del landscape normal
        this.flippedLayout = (type === "landscape-secondary");
      } else if (typeof window.orientation === "number") {
        // Soporte legacy para iOS
        this.flippedLayout = (window.orientation === -90);
      }
    }

    recalcularCoordenadasLayout() {
      if (this.flippedLayout) {
        // MODO FLIPPED (Alternativas a la izquierda, Tablero a la derecha)
        this.boardOffset = { x: 460, y: 130 };
        this.botones = {
          iniciar: { x1: 40, y1: 630, x2: 155, y2: 680, label: "Iniciar" },
          sonido: { x1: 170, y1: 630, x2: 285, y2: 680, label: "Sonido" },
          reiniciar: { x1: 300, y1: 630, x2: 420, y2: 680, label: "Reiniciar" },
          fullscreen: { x1: 460, y1: 630, x2: 650, y2: 680, label: "Pantalla completa" }
        };
        // Posiciones HUD
        this.hudPositions = {
          puntaje: { x: 55, y: 32, w: 120 },
          vidas: { x: 185, y: 32, w: 110 },
          nivel: { x: 305, y: 32, w: 115 },
          progreso: { x: 780, y: 637, w: 150 },
          racha: { x: 940, y: 637, w: 105 }
        };
        // Leyenda
        this.legendCoords = { x: 40, y: 130, w: 380, h: 450 };
        // Enunciado matemático
        this.enunciadoX = 460;
      } else {
        // MODO NORMAL (Tablero a la izquierda, Alternativas a la derecha)
        this.boardOffset = { x: 40, y: 130 };
        this.botones = {
        ...
        // Posiciones HUD
        this.hudPositions = {
          puntaje: { x: 875, y: 32, w: 120 },
          vidas: { x: 1005, y: 32, w: 110 },
          nivel: { x: 1125, y: 32, w: 115 },
          progreso: { x: 360, y: 637, w: 150 },
          racha: { x: 520, y: 637, w: 105 }
        };
        // Leyenda
        this.legendCoords = { x: 860, y: 130, w: 380, h: 450 };
        // Enunciado matemático
        this.enunciadoX = 40;
      }
    }
    ...
    manejarCambioFullscreen() {
      ...
      // Intentar forzar/bloquear modo apaisado (landscape) en pantalla completa en dispositivos móviles
      if (activa) {
        if (screen.orientation && typeof screen.orientation.lock === "function") {
          screen.orientation.lock("landscape").catch(function (error) {
            // Silenciar o registrar advertencia (ej. en computadores de escritorio no se permite bloquear la orientación)
            console.log("Orientación apaisada no bloqueada (normal en computadores):", error.message);
          });
        }
      } else {
        // Desbloquear al salir de pantalla completa
        if (screen.orientation && typeof screen.orientation.unlock === "function") {
          try {
            screen.orientation.unlock();
          } catch (e) {
            console.log("No se pudo desbloquear la orientación:", e.message);
          }
        }
      }
    ...
    iniciar() {
      ...
      // Detectar y recalcular orientación inicial al arrancar el motor
      this.detectarOrientacionFlipped();
      this.recalcularCoordenadasLayout();

      // Escuchar rotación física de 180° del dispositivo móvil para reflejar la interfaz (diestros/zurdos)
      if (screen.orientation) {
        this._evtOrientation = () => {
          this.detectarOrientacionFlipped();
          this.recalcularCoordenadasLayout();
          this.dibujar();
        };
        screen.orientation.addEventListener("change", this._evtOrientation);
      } else {
        this._evtOrientationChange = () => {
          this.detectarOrientacionFlipped();
          this.recalcularCoordenadasLayout();
          this.dibujar();
        };
        window.addEventListener("orientationchange", this._evtOrientationChange);
      }
    ...
    destruir() {
      ...
      // Limpieza de listeners de orientación para evitar fugas de memoria
      if (this._evtOrientation && screen.orientation) {
        screen.orientation.removeEventListener("change", this._evtOrientation);
      }
      if (this._evtOrientationChange) {
        window.removeEventListener("orientationchange", this._evtOrientationChange);
      }
    ...
    dibujar() {
      ...
      // Draw math equation on the top left (drawn on highest layer so it overlaps HUD/buttons if long)
      if (this.preguntaActual) {
        this.dibujarTextoMatematico(ctx, this.preguntaActual.enunciado, this.enunciadoX, 50, 42, "#ffffff", "left");
      }
    ...
    dibujarHUD(ctx) {
      const pos = this.hudPositions;
      // 1. Puntaje: (Top capsule 1)
      this.dibujarPildoraHUD(ctx, pos.puntaje.x, pos.puntaje.y, pos.puntaje.w, 36, "Puntaje", String(this.puntaje), "#ffb04a");
      // 2. Vidas: (Top capsule 2)
      this.dibujarPildoraHUD(ctx, pos.vidas.x, pos.vidas.y, pos.vidas.w, 36, "Vidas", String(this.vidas), "#ffb04a");
      // 3. Nivel: (Top capsule 3)
      this.dibujarPildoraHUD(ctx, pos.nivel.x, pos.nivel.y, pos.nivel.w, 36, "Nivel", String(this.preguntaActual ? obtenerNivel(this.preguntaActual) : "-"), "#ffb04a");
      
      // 4. Progreso: (Bottom capsule 1)
      const progVal = `${this.preguntasResueltas}/${this.preguntasTotales}`;
      this.dibujarPildoraHUD(ctx, pos.progreso.x, pos.progreso.y, pos.progreso.w, 36, "Progreso", progVal, "#ffb04a");
      // 5. Racha: (Bottom capsule 2)
      this.dibujarPildoraHUD(ctx, pos.racha.x, pos.racha.y, pos.racha.w, 36, "Racha", String(this.rachaActual), "#ffb04a");
    }
    ...
    dibujarObjetosEnLeyenda(ctx) {
      ...
      const lx = this.legendCoords.x;
      const ly = this.legendCoords.y;
      const lw = this.legendCoords.w;
      const lh = this.legendCoords.h;
