...
(function (window, document) {
  ...
  function isCurveAreaRawType(type) {
    return type === 'circle' || type === 'circle-radius' || type === 'ellipse';
  }
  ...
  */
  ...
  function loadSceneFromOptions(options = {}) {
    ...
    if (options.target || options.container) {
      const embedded = readEmbeddedSceneText(resolveSceneSourceElement(options.target || options.container));
...
.geo2d-notable-visible {
  ...
  class Geo2DHitTester {
    ...
    getObjectHitDistance(sx, sy, resolved, obj, vp, includeInterior = true) {
      ...
      if (resolved.kind === 'circle') {
        const cs = circleScreenRadius(vp, this.width, this.height, resolved.center.x, resolved.center.y, resolved.radius);
        const inside = dist2(sx, sy, cs.cx, cs.cy) <= cs.r * cs.r;
  ...
  class Geo2DConstructionControllerV2 {
    ...
    dispatchClick(world, sx, sy) {
      switch (this.activeTool) {
        ...
        case 'number':
          return this.handleNumberClick(world, sx, sy);
        case 'text':
        ...
        case 'regular-polygon':
          return this.handleRegularPolygonClick(world, sx, sy);
        case 'intersect':
    ...
    promptIndependentNumberConfig() {
      let previousValue = '5';
      let previousStep = '1';
      let value = null;

      while (true) {
        const rawValue = prompt('Valor del numero:', previousValue);
        if (rawValue === null) return null;
        previousValue = String(rawValue).trim() || previousValue;
        value = safeNumber(String(rawValue).replace(',', '.'), NaN);
        if (Number.isFinite(value)) break;
      }

      while (true) {
        const rawStep = prompt('Paso del numero:', previousStep);
        if (rawStep === null) return null;
        previousStep = String(rawStep).trim() || previousStep;
        const step = safeNumber(String(rawStep).replace(',', '.'), NaN);
        if (step > 0) {
          return { value, step };
        }
      }
    ...
    handleNumberClick(_world, _sx, _sy) {
      const editor = this.editor;
      const config = this.promptIndependentNumberConfig();
      if (!config) {
        editor.setStatus('Numero cancelado.');
        return;
      }
    ...
    handleEllipseClick(world, sx, sy) {
      ...
      if (coVertexId) {
        ...
        const alreadyOnAxis = nearPosition && dist(nearPosition.x, nearPosition.y, projectedCoVertex.x, projectedCoVertex.y) <= 1e-7;
    ...
    handleCircleRadiusClick(world, sx, sy) {
      ...
      const rawRadius = prompt('Radio de la circunferencia o id de numero:', '5');
      if (rawRadius === null) {
        editor.setStatus('Circunferencia (C,r) cancelada.');
        return;
      }

      const radiusToken = String(rawRadius).trim();
      if (!radiusToken) {
        editor.setStatus('Debes indicar un radio o el id de un numero.', true);
        return;
      }
      ...
      const existingRadiusObject = this.model.getObject(radiusToken);
      if (existingRadiusObject) {
        if (InternalObjectAdapter.type(existingRadiusObject) !== 'number') {
          editor.setStatus(`"${radiusToken}" existe, pero no es un numero.`, true);
          return;
        }
        circleObjectByRadius.radiusRef = radiusToken;
      } else {
        const parsedRadius = safeNumber(radiusToken.replace(',', '.'), NaN);
        if (!(parsedRadius > 1e-9)) {
          editor.setStatus(`No existe un numero con id "${radiusToken}" y tampoco es un radio valido.`, true);
          return;
        }
        circleObjectByRadius.radius = parsedRadius;
      }
    ...
    promptRegularPolygonSides() {
      let previous = '5';

      while (true) {
        const rawSides = prompt('Número de lados del polígono regular:', previous);
        if (rawSides === null) return null;

        previous = String(rawSides).trim() || previous;
        const sides = Math.floor(safeNumber(String(rawSides).replace(',', '.'), NaN));
        if (sides >= 3) return sides;
      }
    }

    handleRegularPolygonClick(world, sx, sy) {
      const editor = this.editor;
      ...
      const sides = this.promptRegularPolygonSides();
      if (sides === null) {
        if (picked.created) {
      ...
      if (editor._pendingPoints.length === 2) {
        ...
        while (true) {
          ...
          if (existingObject) {
            ...
            try {
              angleDef = {
                measureValue: validateAngleMeasureValue(numericValue, 'deg', 'angulo')
              };
            } catch (error) {
    ...
    handlePointSequenceClick(world, sx, sy) {
      ...
      if (this.activeTool === 'angle-measure') {
        this.handleAngleByMeasurePoint();
        return;
      }

      if (this.activeTool === 'bisector-ray') {
        this.handleBisectorRayPoint();
        return;
      }
  ...
  class Geo2DEditor {
    ...
    nudgeSelectedNumber(direction) {
      ...
      obj.raw.value = safeNumber(obj.raw.value, 0) + step * dir;
      this.renderAndSync();
      return true;
    }
    ...
    getEllipseDragAdjustmentsForPoint(pointId) {
      ...
      for (const obj of this.model.objects) {
        ...
        const vertexRadius = dist(center.x, center.y, vertex.x, vertex.y);
        const coVertexRadius = dist(center.x, center.y, coVertex.x, coVertex.y);
  ...
  class Geo2DBuilder {
    ...
    number(id, options = {}) {
      ...
      if (rawKind === 'independent') {
        this.pushObject(applyBuilderPresentation({
          ...
          def: {
            kind: 'independent',
            value: safeNumber(config.value, NaN),
    ...
    intersection(id, options = {}) {
      ...
      const def = {
        ...
        hint: {
          x: safeNumber(options.x ?? options.hint?.x, 0),
          y: safeNumber(options.y ?? options.hint?.y, 0)
        }
      };
      copyOptionalEdgeIndex(options, def);
  ...
  function resolvePublicTarget(target, fallbackSelector) {
    if (!target && fallbackSelector) return resolveSceneSourceElement(fallbackSelector);
    return resolveSceneSourceElement(target);
  }
  ...
  function publishHtml(sceneLike, options = {}) {
    ...
    const className = String(options.className || 'geo2d-viewer').trim() || 'geo2d-viewer';
    const sceneText = serializeSceneForHtmlBlock(scene);
    return `<div class="${escapeHtml(className)}" id="${escapeHtml(id)}" data-geo2d-viewer data-viewer-mode="${mode}"><textarea data-geo2d-scene style="display:none;">${sceneText}</textarea></div>`;
  }
