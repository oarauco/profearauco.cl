    ...
    if (state._pendingPoints.length > 0 && previewAnchor) {
      ...
      if (['polyline', 'area', 'polygon'].includes(state.activeTool)) {
        ...
        if (pts.length > 0) {
          ...
          const previewScreen = worldToScreen(vp, width, height, previewAnchor.x, previewAnchor.y);
          const previewPts = [...screenPts, previewScreen];
...
if (this.activeTool === 'move') {
  ...
  return;
      ...
      // Fallback robusto para cerrar figuras cuando el evento dblclick no
      // llega de forma consistente en SVG/pointer events.
      if (
        this.isFigureTool() &&
        e.detail >= 2 &&
        this._pendingPoints.length >= this.minPointsForFigureTool()
    ...
    handleConstructionClick(world, sx, sy) {
      ...
      if (this.isFigureTool()) {
        ...
        this.setStatus(
          this.activeTool === 'polyline'
            ? `Poligonal: ${this._pendingPoints.length} punto(s). Doble clic para terminar.`
            : `${this.getFigureToolLabel()}: ${this._pendingPoints.length} punto(s). Haz clic en el punto inicial o doble clic para cerrar.`
        );
        return;
      }
