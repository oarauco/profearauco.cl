(function (window) {
  ...
  function normalizarModoDidactico(opciones) {
    var opts = opciones || {};
    var raw = DC.valorValido(opts.modo) ? opts.modo : opts.mode;
    var modo = DC.aTexto(raw, '').trim().toLowerCase();

    if (!modo && opts.mostrarSolucion === false) modo = 'ejercicio';
    if (!modo && opts.mostrarSolucion === true) modo = 'solucion';
    if (!modo) return '';

    if (modo === 'enunciado' || modo === 'planteamiento' ||
        modo === 'evaluacion' || modo === 'evaluación' ||
        modo === 'prueba') {
      return 'ejercicio';
    }

    if (modo === 'guia' || modo === 'guía' || modo === 'parcial') {
      return 'guiado';
    }

    if (modo === 'resuelto' || modo === 'respuesta' ||
        modo === 'explicacion' || modo === 'explicación') {
      return 'solucion';
    }

    if (modo === 'ejercicio' || modo === 'guiado' || modo === 'solucion') {
      return modo;
    }
  ...
  function filtrarLineasSolucion(lines) {
    if (!Array.isArray(lines)) return lines;

    return lines.filter(function (linea) {
      var txt = DC.aTexto(linea, '').trim().toLowerCase();
      if (!txt) return false;
      return !(
        txt.indexOf('p(total)') === 0 ||
        txt.indexOf('probabilidad total') === 0 ||
        txt.indexOf('p total') === 0
      );
    });
  }

  function recorrerNodos(nodo, fn) {
    if (!nodo) return;
    fn(nodo);
    (nodo.children || []).forEach(function (child) {
      recorrerNodos(child, fn);
    });
  }

  function aplicarModoDidactico(cfg, opciones) {
    var modo = normalizarModoDidactico(opciones);
    if (!modo) return cfg;

    cfg.modoDidactico = modo;
    cfg.mostrarSolucion = modo === 'solucion';

    if (modo === 'ejercicio') {
      cfg.leafBoxStyle = cfg.leafBoxStyle || {};
      cfg.leafBoxStyle.show = false;

      recorrerNodos(cfg.root, function (node) {
        node.leafBoxStyle = DC.mergeProfundo(node.leafBoxStyle || {}, { show: false });
      });
    ...
    if (modo === 'guiado') {
      recorrerNodos(cfg.root, function (node) {
        if (node.leafBox && Array.isArray(node.leafBox.lines)) {
          node.leafBox.lines = filtrarLineasSolucion(node.leafBox.lines);
        }
      });
    }
  ...
  function crearConfigComun(opciones, extra) {
    ...
    cfg = aplicarModoDidactico(cfg, opts);
...
function normalizarRamaGenerica(rama, idx, estiloBase) {
  ...
  var api = {
    ...
    normalizarModoDidactico: normalizarModoDidactico,
    aplicarModoDidactico: aplicarModoDidactico,
