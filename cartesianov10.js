...
(function (window, document) {
  ...
  function detectPlotType(expr) {
    ...
    if (findTopLevelRelationOperator(normalized)) return 'relation';
  ...
  function preparePointSpec(spec) {
    const plotType = detectPlotType(spec.expr);
    return Object.assign({}, spec, {
      plotType,
      residualExpr: plotType === 'relation'
        ? buildRelationResidualExpr(spec.expr)
        : null
    });
  }
  ...
  function drawDot(svg, map, x, y, color, pointDecimals) {
    const offset = GRAPH_SETTINGS.pointLabelOffsetPx;
    ...
    t.setAttribute('x', map.x2px(x) + offset);
    t.setAttribute('y', map.y2px(y) - offset);
  ...
  function drawExplicitPoints(svg, map, state) {
    ...
    pts.forEach(p => {
      if (
        ...
        drawDot(svg, map, p.x, p.y, '#111', state.pointDecimals);
      }
    });
  }
    ...
    spec.slice(0, 16).forEach(s => {
      ...
      ys.forEach(y => {
        if (y >= state.ymin && y <= state.ymax) {
          drawDot(svg, map, s.x0, y, '#111', state.pointDecimals);
        }
      });
    });
  }
    ...
    spec.forEach(s => {
      ...
      xs.forEach(xi => {
        if (xi >= state.xmin && xi <= state.xmax) {
          drawDot(svg, map, xi, s.y0, '#111', state.pointDecimals);
        }
      });
    });
  }
  ...
  function drawPointIntersections(svg, map, state) {
    ...
    pairs.forEach(pair => {
      ...
      points.forEach(point => {
        ...
        drawn.push(point);
        drawDot(svg, map, point.x, point.y, '#111', state.pointDecimals);
      });
    });
  }
...
if (!appendAdaptiveInterval(xm, ym, x2, y2, out, depth + 1, strictClipCheck)) return false;
  ...
  function drawRelationContour(svg, residualExpr, map, xmin, xmax, ymin, ymax, color) {
    if (!residualExpr) return;

    const targetCellPx = 6;
    const cols = Math.max(48, Math.min(240, Math.ceil(map.w / targetCellPx)));
    const rows = Math.max(36, Math.min(240, Math.ceil(map.h / targetCellPx)));
  ...
  function redrawOne(svg, state) {
    ...
    if (svg.getAttribute('data-legend') === 'true') {
      drawLegend(svg, state.curves || (state.funcs || []).concat(state.relations || []));
    }
...
function attachInteractions(svg, state, onChange) {
  ...
  function initOne(svg) {
    ...
    exprs.forEach((expr, i) => {
      const plotType = detectPlotType(expr);
      ...
      curves.push({
        ...
        expr,
        plotType,
        residualExpr: plotType === 'relation' ? buildRelationResidualExpr(expr) : null,
        label: labels[i] || expr,
        color: plotType === 'relation'
          ? (colors[i] || '#d32f2f')
          : (colors[i] || undefined),
        curveId,
    ...
    relationExprs.forEach((expr, i) => {
      ...
      curves.push({
        ...
        expr,
        plotType: 'relation',
        residualExpr: buildRelationResidualExpr(expr),
