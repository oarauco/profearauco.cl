...
function definirEventoCompuesto(instancia, nombre, spec, meta) {
  ...
  function resolverEventoEjercicio(instancia, opciones) {
    var opts = opciones || {};
    var nombre = DC.aTexto(opts.nombre || opts.evento || 'Evento favorable', 'Evento favorable');
    var evento;
    var resumen;
    var prob;
    var textoProb;
    ...
    if (opts.evento && obtenerEvento(instancia, opts.evento)) {
      evento = obtenerEvento(instancia, opts.evento);
    } else if (typeof opts.criterio === 'function') {
      evento = definirEvento(instancia, nombre, opts.criterio, opts.meta || {});
    } else if (DC.esObjetoPlano(opts.expresion)) {
      evento = definirEventoDesdeExpresion(instancia, nombre, opts.expresion, opts.meta || {});
    } else if (DC.esObjetoPlano(opts.compuesto)) {
      evento = definirEventoCompuesto(instancia, nombre, opts.compuesto, opts.meta || {});
    } else {
      throw new Error('DiarbolEventos.resolverEventoEjercicio: falta criterio, expresion, compuesto o evento existente.');
    }

    resumen = resumenEvento(instancia, evento);
    prob = resumen.probabilidad;
    textoProb = prob ? prob.texto : '?';
    texto = 'Casos favorables: ' + resumen.hojasFavorables + ' de ' + resumen.hojasTotales +
      '. P(' + resumen.nombre + ') = ' + textoProb;

    if (opts.resaltar !== false) {
      resaltarEvento(instancia, evento, opts.resaltado || opts.highlight || {});
    }
    ...
    return {
      ...
      probabilidad: prob,
      texto: texto,
      hojasFavorables: resumen.hojasFavorables,
      hojasTotales: resumen.hojasTotales,
      instancia: instancia
    };
  }
  ...
  var api = {
   ...
   definirEventoCompuesto: definirEventoCompuesto,
    ...
    resolverEventoEjercicio: resolverEventoEjercicio,
